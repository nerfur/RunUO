	...
	public class Stealing
	{
		...
		private class StealingTarget : Target
		{
			...
			private Item TryStealItem( Item toSteal, ref bool caught )
			{
				...
				{
					m_Thief.SendAsciiMessage( "Both hands must be free to steal." ); // Both hands must be free to steal.
				}
				else if ( root is Mobile && ((Mobile)root).Player && IsInnocentTo( m_Thief, (Mobile)root ) && !IsInGuild( m_Thief ) )
				{
                    m_Thief.SendAsciiMessage("You must be in the thieves guild to steal from other players."); // You must be in the thieves guild to steal from other players.
				}
				else if ( SuspendOnMurder && root is Mobile && ((Mobile)root).Player && IsInGuild( m_Thief ) && m_Thief.Kills > 0 )
				{
                    m_Thief.SendAsciiMessage("You are currently suspended from the thieves guild."); // You are currently suspended from the thieves guild.
				}
				/*else if ( root is BaseVendor && ((BaseVendor)root).IsInvulnerable )
				{
                    m_Thief.SendAsciiMessage("You can't steal from shopkeepers."); // You can't steal from shopkeepers.
				}*/
				else if ( root is PlayerVendor )
				{
                    m_Thief.SendAsciiMessage("You can't steal from vendors."); // You can't steal from vendors.
				}
				else if ( !m_Thief.CanSee( toSteal ) )
				{
                    m_Thief.SendAsciiMessage("Target can not be seen."); // Target can not be seen.
				}
				else if ( m_Thief.Backpack == null || !m_Thief.Backpack.CheckHold( m_Thief, toSteal, false, true ) )
				{
                    m_Thief.SendAsciiMessage("Your backpack can't hold anything else."); // Your backpack can't hold anything else.
				}
				#region Sigils
				...
				{
                    m_Thief.SendAsciiMessage("You can't steal that!"); // You can't steal that!
				}
				...
				{
                    m_Thief.SendAsciiMessage("You must be standing next to an item to steal it."); // You must be standing next to an item to steal it.
				}
				...
				{
                    m_Thief.SendAsciiMessage("You catch yourself red-handed."); // You catch yourself red-handed.
				}
				...
				else
				{
					...
					if ( w > 10 )
					{
                        m_Thief.SendAsciiMessage("This item is too heavy to steal from someone's backpack.");
					}
					else
					{
						...
						if ( stolen != null )
						{
							m_Thief.SendAsciiMessage( "You succesfully steal the item." ); // You succesfully steal the item.
						...
						{
                            m_Thief.SendAsciiMessage("You fail to steal the item."); // You fail to steal the item.
						}
			...
			protected override void OnTarget( Mobile from, object target )
			{
				...
				else if ( target is Mobile )
				{
					...
					if ( pack != null && pack.Items.Count > 0 )
					{
                        ...
                        from.SendAsciiMessage("You reach into " + ((Mobile)target).Name + "'s backpack... and try to take something.");

						root = target;
						stolen = TryStealItem( pack.Items[randomIndex], ref caught );
					}
                    else
                        from.SendAsciiMessage("You reach into " + ((Mobile)target).Name + "'s backpack... but find it's empty.");
				} 
				else 
				{
					m_Thief.SendAsciiMessage( "You can't steal that!" ); // You can't steal that!
		...
		public static TimeSpan OnUse( Mobile m )
		{
			if ( !IsEmptyHanded( m ) )
			{
				m.SendAsciiMessage( "Both hands must be free to steal." ); // Both hands must be free to steal.
			}
			else
			{
				...
				m.SendAsciiMessage( "Which item do you want to steal?" ); // Which item do you want to steal?
			}
