...
namespace Server.SkillHandlers
{
	public class Tracking
	{
		...
		{
            //m.SendAsciiMessage("What do you wish to track?");
            ...
            m.SendMenu(new TrackWhatMenu(m, TrackWhatMenu.Main()));
    ...
    public class TrackWhatMenu : ItemListMenu
    {
        private Mobile m_From;
        private bool m_Success;

        public TrackWhatMenu( Mobile from, ItemListEntry[] entries ) : base("What do you wish to track?", entries)
        {
            m_From = from;
    ...
    public class TrackWhoMenu : ItemListMenu
    {
        ...
        public TrackWhoMenu(Mobile from, ItemListEntry[] entries, List<Mobile> list, int range) : base("What do you wish to track?", entries)
        {
            m_From = from;
            m_List = list;
            m_Range = range;
	...
	public class TrackWhoGump : Gump
	{
		...
		public static void DisplayTo( bool success, Mobile from, int type )
		{
			if ( !success )
			{
				from.SendAsciiMessage( "You see no evidence of those in the area." ); // You see no evidence of those in the area.
				return;
			}
			...
			if ( list.Count > 0 )
			{
                ...
                from.SendMenu(new TrackWhoMenu(from, TrackWhoMenu.Main(list, from), list, range));
				//from.SendLocalizedMessage( 1018093 ); // Select the one you would like to track.
			}
			else
			{
				if ( type == 0 )
					from.SendAsciiMessage( "You see no evidence of animals in the area." ); // You see no evidence of animals in the area.
				else if ( type == 1 )
                    from.SendAsciiMessage("You see no evidence of creatures in the area."); // You see no evidence of creatures in the area.
				else
                    from.SendAsciiMessage("You see no evidence of people in the area."); // You see no evidence of people in the area.
			}
		}
