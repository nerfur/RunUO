...
namespace Server.SkillHandlers
{
	public class Poisoning
	{
		...
		public static TimeSpan OnUse( Mobile m )
		{
			...
			//m.SendLocalizedMessage( 502137 ); // 
            m.SendAsciiMessage("Select the poison you wish to use.");
				...
				protected override void OnTarget( Mobile from, object targeted )
				{
					...
					else // Target can't be poisoned
					{
						if ( Core.AOS )
							...
							from.SendAsciiMessage( "You cannot poison that! You can only poison bladed or piercing weapons, food or drink." ); // You cannot poison that! You can only poison bladed or piercing weapons, food or drink.
					}
				}
				...
				private class InternalTimer : Timer
				{
					...
					protected override void OnTick()
					{
						if ( m_From.CheckTargetSkill( SkillName.Poisoning, m_Target, m_MinSkill, m_MaxSkill ) )
						{
							...
							m_From.SendAsciiMessage( "You apply the poison." ); // You apply the poison
						...
						else // Failed
						{
							...
							{
                                m_From.SendAsciiMessage("You make a grave mistake while applying the poison."); // You make a grave mistake while applying the poison.
							...
							else
							{
								if ( m_Target is BaseWeapon )
								{
									...
									if ( weapon.Type == WeaponType.Slashing )
										m_From.SendAsciiMessage( "You fail to apply a sufficient dose of poison on the blade." ); // You fail to apply a sufficient dose of poison on the blade
									else
										m_From.SendAsciiMessage( "You fail to apply a sufficient dose of poison." ); // You fail to apply a sufficient dose of poison
								}
								else
								{
                                    m_From.SendAsciiMessage("You fail to apply a sufficient dose of poison.");  // You fail to apply a sufficient dose of poison
