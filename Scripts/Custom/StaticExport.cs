 ...
 *************************
 */
// with this script i fell myself a genius! ;-P
//
using System;
using System.IO;
using System.Text;
using System.Collections;
using System.Collections.Generic; //Dictionary support
using Server;
...
namespace Server.Commands
{
	public class StaticExport
	{
		...
		private static void Export( Mobile from, string file, int X1, int Y1, int X2, int Y2 )
		{
			...
			Dictionary<string, ArrayList> DicOfItemIDs = new Dictionary<string, ArrayList>();
			...
			using ( StreamWriter op = new StreamWriter( String.Format( @".\Export\{0}.cfg", file ) ) )
			{
				...
				foreach ( Item item in list )
				{
					...
					else
					{
						...
						{
							ArrayList CoordXYZupdated = DicOfItemIDs[NamePlusID];
						...
						{
							if ( itemname != "AddonComponent" ) // ignore AddonComponent
							{
								ArrayList CoordXYZ = new ArrayList();
								CoordXYZ.Add(Coord);
				...
				foreach ( KeyValuePair<string, ArrayList> pair in DicOfItemIDs )
				{
