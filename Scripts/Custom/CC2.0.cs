    ...
    class ServerConsole
    {
        ...
        public static void BroadcastMessage(AccessLevel ac, int hue, string message)
        {
            foreach (NetState state in NetState.Instances)
            {
                ...
                if (m != null && m.AccessLevel >= ac)
                    ...
                    m.Send(new AsciiMessage(Serial.MinusOne, -1, MessageType.Regular, hue, 0, "", message));
            }
        }
        public static void Next(string nput)
        {
            ...
            if (input.StartsWith("bc"))
            {
                ...
                BroadcastMessage(AccessLevel.Player, 0x3B2, String.Format("[System]:{0}", imput));
                Console.WriteLine("Players will see: {0}", imput);
            }
            else if (input.StartsWith("sc"))
            {
                string imput = input.Replace("sc", "");
                BroadcastMessage(AccessLevel.Counselor, 0x3B2, String.Format("[System]:{0}", imput));
                    ...
                    case "restart":
                        {
                            ...
                            World.Broadcast(0x3B2, true, "[System]: The server is going down shortly.");
                            Misc.AutoSave.Save();
                            World.Broadcast(0x3B2, true, "[System]: The server has shut down.");
