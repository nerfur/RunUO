...
namespace Server
{
    ...
    {
        public static string FarewellLow(BaseCreature m_Mobile, Mobile from)
        {
            ...
            else if (from.Karma >= 60)
            {
                ...
                else if (m_Mobile.Attitude == AttitudeLevel.Neutral || m_Mobile.Attitude == AttitudeLevel.Goodhearted)
                {
                    switch (Utility.Random(3))
                    {
                        case 0: response = String.Format("An honor to speak with ye, {0}.", from.Name); break;
                        case 1: response = String.Format("Farewell, {0}.", from.Name); break;
                        case 2: response = String.Format("Bye, {0}.", from.Name); break;
                    }
                }
            }
            //Annonymous
