...
namespace Server
{
	public class Loot
	{
        ...
        //Specific Armor
        private static Type[] m_BoneArmorTypes = new Type[]
			{
				typeof( BoneArms ),				typeof( BoneChest ),			typeof( BoneGloves ),
				typeof( BoneLegs ),				typeof( BoneHelm )
			};
        public static Type[] BoneArmorTypes { get { return m_BoneArmorTypes; } }

        private static Type[] m_ChainArmorTypes = new Type[]
			{
                typeof( ChainChest ),           typeof( OrcHelm ),
				typeof( ChainLegs ),			typeof( ChainCoif )
			};
        public static Type[] ChainArmorTypes { get { return m_ChainArmorTypes; } }

        private static Type[] m_PlateArmorTypes = new Type[]
			{
                typeof( Bascinet ),
				typeof( CloseHelm ),			typeof( Helmet ),				typeof( NorseHelm ),
        ...
        public static Type[] PlateArmorTypes { get { return m_PlateArmorTypes; } }
        ...
        private static Type[] m_LeatherArmorTypes = new Type[]
			{
				...
				typeof( LeatherBustierArms ),	typeof( LeatherChest ),			typeof( LeatherGloves ),
				typeof( LeatherGorget ),		typeof( LeatherLegs ),			typeof( LeatherShorts ),
				typeof( LeatherSkirt ),			typeof( LeatherCap )
			};
        public static Type[] LeatherArmorTypes { get { return m_LeatherArmorTypes; } }

        private static Type[] m_RingmailArmorTypes = new Type[]
			{
                typeof( RingmailArms ),			typeof( RingmailChest ),		typeof( RingmailGloves ),
				typeof( RingmailLegs )
			};
        public static Type[] RingmailArmorTypes { get { return m_RingmailArmorTypes; } }

        private static Type[] m_StuddedArmorTypes = new Type[]
			{
                typeof( FemaleStuddedChest ),	typeof( StuddedArms ),
				typeof( StuddedBustierArms ),	typeof( StuddedChest ),			typeof( StuddedGloves ),
				typeof( StuddedGorget ),		typeof( StuddedLegs )
			};
        public static Type[] StuddedArmorTypes { get { return m_StuddedArmorTypes; } }
        //End Armor types
        ...
        private static Type[] m_StationaryTypes = new Type[]
		{
			typeof( Statue1 ),			typeof( Statue4 ),
			typeof( Statue2 ),			typeof( Statue5 ),
			typeof( Statue3 ),		    typeof( MagicBrazier )
		};

        public static Type[] StationaryTypes { get { return m_StationaryTypes; } }
		...
		private static Type[] m_ClothingTypes = new Type[]
			{
				...
				typeof( HalfApron ),            typeof( BearMask ),		        typeof( DeerMask ),
                typeof( TribalMask ),           typeof( OrcMask ),              typeof( HornedTribalMask )
			};
		...
		private static Type[] m_HatTypes = new Type[]
			{
                ...
                typeof( BearMask ),		    typeof( DeerMask ),     typeof( TribalMask ),
                typeof( HornedTribalMask ), typeof( TribalMask ),   typeof( OrcMask )
			};
        ...
        public static BaseBeverage RandomBeverage()
        {
            ...
            {
                default:
                case 0: return new BeverageBottle(BeverageType.Wine);
                case 1: return new BeverageBottle(BeverageType.Ale);
                case 2: return new BeverageBottle(BeverageType.Cider);
                case 3: return new BeverageBottle(BeverageType.Liquor);
            }
        ...
        public static BaseClothing RandomMagicClothing()
        {
            ...
            {
                item.Effect = ClothEffect.NightSight;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (46 >= random)
            {
                item.Effect = ClothEffect.Protection;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (54 >= random)
            {
                item.Effect = ClothEffect.Agility;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (62 >= random)
            {
                item.Effect = ClothEffect.Cunning;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (70 >= random)
            {
                item.Effect = ClothEffect.Strength;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (78 >= random)
            {
                item.Effect = ClothEffect.Invisibility;
                item.Charges = Utility.RandomMinMax(1, 5);
            }
            else if (83 >= random)
            {
                item.Effect = ClothEffect.MagicReflection;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (87 >= random)
            {
                item.Effect = ClothEffect.Feeblemind;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (91 >= random)
            {
                item.Effect = ClothEffect.Clumsy;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (95 >= random)
            {
                item.Effect = ClothEffect.Weaken;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else if (98 >= random)
            {
                item.Effect = ClothEffect.Bless;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            else
            {
                item.Effect = ClothEffect.Curse;
                item.Charges = Utility.RandomMinMax(10, 31);
            }
            #endregion
		...
		public static BaseArmor RandomArmor( bool inTokuno )
		{
            ...
            {
                return Construct(m_BoneArmorTypes) as BaseArmor;
            }
		...
		{
			return Construct( m_StationaryTypes );
		}
