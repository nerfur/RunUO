...
namespace Server.Misc
{
	public class TestCenter
	{
		...
		private static void ChangeStrength( Mobile from, int value )
		{
			...
			{
				from.SendAsciiMessage( "Stats range between 10 and 100." ); // Stats range between 10 and 125.
			}
			else
			{
				if ( (value + from.RawDex + from.RawInt) > from.StatCap )
				{
					from.SendAsciiMessage( "You can not exceed the stat cap.  Try setting another stat lower first." ); // You can not exceed the stat cap.  Try setting another stat lower first.
				}
				else
				{
					from.RawStr = value;
					from.SendAsciiMessage( "Your stats have been adjusted." ); // Your stats have been adjusted.
				}
			}
		}
		...
		private static void ChangeSkill( Mobile from, string name, double value )
		{
			...
			{
				from.SendAsciiMessage( "You have specified an invalid skill to set." ); // You have specified an invalid skill to set.
				return;
			}
            ...
            if (skill != null)
            {
                ...
                {
                    value = 100;
                    //from.SendAsciiMessage( String.Format( "Your skill in {0} is capped at {1:F1}.", skill.Info.Name, skill.Cap ) );
                }
                ...
                {
                    from.SendAsciiMessage("You can not exceed the skill cap.  Try setting another skill lower first.");
                }
                else
                {
