...
namespace Server.Misc
{
	public class NotorietyHandlers
	{
		...
		public static int MobileNotoriety( Mobile source, Mobile target )
		{
            ...
            if (target is BaseCreature && !(target.Body.IsMonster) && IsSummoned(target as BaseCreature))
            {
            ...
            //Old Style Karma Checks
            if (target.Karma < -59 && target is PlayerMobile)
                return Notoriety.Murderer;
            ...
            //Old Style Karma Checks
            if (((target.Karma < -39) && (target.Karma > -60)) && target is PlayerMobile)
                return Notoriety.Criminal;
            ...
            {
                if ( ((target.Body.IsMonster) && IsSummoned(target as BaseCreature) && !(target is BaseFamiliar) && !(target is ArcaneFey) && !(target is Golem)) || (target is BaseCreature && (((BaseCreature)target).AlwaysMurderer || ((BaseCreature)target).IsAnimatedDead)))
                    return Notoriety.Murderer;
            }*/
			...
			if( target is BaseCreature )
			{
				...
				if( bc.Controlled && bc.ControlOrder == OrderType.Guard && bc.ControlTarget == source )
					return Notoriety.CanBeAttacked;
                else if (IsPet(target as BaseCreature) && master != null && !IsSummoned(target as BaseCreature))
                {
