	...
	public class Guild : BaseGuild
	{
		...
		private static void GuildProps_OnCommand( CommandEventArgs e )
		{
			...
			else
			{
				...
				int id = Utility.GetXMLInt32( arg, -1 );
				if( id != -1 )
		...
		public void Disband()
		{
			...
			{
				m.SendAsciiMessage( "Your guild has disbanded." ); // Your guild has disbanded.
		...
		{
			RemoveMember( m, "You have been dismissed from your guild." ); // You have been dismissed from your guild.
		}
		public void RemoveMember( Mobile m, string message )
		{
			if ( m_Members.Contains( m ) )
			{
				m_Members.Remove( m );
		...
		{
			for( int i = 0; i < m_Members.Count; ++i )
				m_Members[i].SendAsciiMessage( hue, text );
		}
		...
		public void CalculateGuildmaster()
		{
			...
			if ( m_Leader != winner && winner != null )
				GuildTextMessage( String.Format("Guild Message: Guildmaster changed to: {0}", winner.Name) ); // Guild Message: Guildmaster changed to:
