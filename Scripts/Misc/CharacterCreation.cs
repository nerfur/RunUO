...
namespace Server.Misc
{
	public class CharacterCreation
	{
		...
		private static void SetStats( Mobile m, NetState state, int str, int dex, int intel )
		{
            ...
            while (((int)strr + (int)dexx + (int)intell) > 65)
            {
                scale -= 0.0001;
                ...
                strr = Math.Ceiling(strr);
                dexx = Math.Ceiling(dexx);
                intell = Math.Ceiling(intell);
		...
		private static bool ValidSkills( SkillNameValue[] skills, Mobile m )
		{
			int total = 0;
		...
		private static void SetSkills( Mobile m, SkillNameValue[] skills, int prof )
		{
            ...
            int total = 0;
            List<SkillName> mylist = new List<SkillName>();

			for ( int i = 0; i < skills.Length; ++i )
			{
				SkillNameValue snv = skills[i];

				if ( snv.Value > 0 && ( snv.Name != SkillName.Stealth || prof == 7 ) && snv.Name != SkillName.RemoveTrap && snv.Name != SkillName.Spellweaving && snv.Name != SkillName.Meditation )
				{
            ...
            while (total > 1000)
            {
                total = 0;
                scale -= 0.0001;
                foreach (SkillName skill in mylist)
                {
                    m.Skills[skill].BaseFixedPoint = (int)(((double)m.Skills[skill].BaseFixedPoint)*scale);
                    total += m.Skills[skill].BaseFixedPoint;
                }
            }
		}
		...
		{
            switch ( /*Utility.Random( 8 ) * (circle * 8)*/Utility.Random(15))
			{
		...
		private static void	AddSkillItems( SkillName skill, Mobile m )
		{
			...
			switch ( skill )
			{
				case SkillName.Alchemy:
				{
                    ...
                    PackItem(MakeNewbie(new BlackPearl(Utility.Random(2, 3))));
                    PackItem(MakeNewbie(new Bloodmoss(Utility.Random(2, 3))));
                    PackItem(MakeNewbie(new Garlic(Utility.Random(2, 3))));
                    PackItem(MakeNewbie(new Ginseng(Utility.Random(2, 3))));
                    PackItem(MakeNewbie(new MandrakeRoot(Utility.Random(2, 3))));
                    PackItem(MakeNewbie(new Nightshade(Utility.Random(2, 3))));
                    PackItem(MakeNewbie(new SulfurousAsh(Utility.Random(2, 3))));
                    PackItem(MakeNewbie(new SpidersSilk(Utility.Random(2, 3))));
                ...
                case SkillName.Tinkering:
                {
                    ...
                    switch (Utility.Random(3))
                    {
                        case 0: PackItem(new Axle()); break;
                        case 1: PackItem(new Gears()); break;
                        case 2: PackItem(new Hinge()); break;
                        case 3: PackItem(new Springs()); break;
                    }
                    switch (Utility.Random(3))
                    {
                        case 0: PackItem(new Axle()); break;
