...
namespace Server.Items
{
	public class TillerMan : Item
	{
        ...
        public void Say(bool ascii, string args)
        {
            PublicOverheadMessage(MessageType.Regular, 0x3B2, ascii, args);
        }
		...
		public override void OnSingleClick( Mobile from )
		{
            if (m_Boat != null && m_Boat.ShipName != null)
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("The tiller man of the {0}", m_Boat.ShipName)));
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", m_Boat.StringStatus));
            }
            else
            {
                if (m_Boat != null)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("a tiller man")));
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", m_Boat.StringStatus));
                }
                else
                {
