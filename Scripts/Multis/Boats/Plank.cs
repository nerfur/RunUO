...
namespace Server.Items
{
	...
	public class Plank : Item, ILockable
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a gang plank"));
            }
        }
			...
			if ( from.InRange( GetWorldLocation(), 8 ) )
			{
				...
				else
				{
					if ( !IsOpen )
					{
						...
						{
							from.LocalOverheadMessage( Network.MessageType.Regular, 0x00, true, "That is locked but your godly powers allow access" ); // That is locked but your godly powers allow access
							Open();
						}
						else
						{
							from.LocalOverheadMessage( Network.MessageType.Regular, 0x00, true, "That is locked." ); // That is locked.
						}
					}
					else if ( !Locked )
					{
