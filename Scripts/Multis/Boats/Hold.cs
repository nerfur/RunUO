...
namespace Server.Items
{
	public class Hold : Container
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a hatch"));
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "(" + this.TotalItems + " items, " + this.TotalWeight + " stones)"));
            }
        }
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( m_Boat == null || !m_Boat.Contains( from ) )
			{
				if ( m_Boat.TillerMan != null )
					m_Boat.TillerMan.Say( true, "You must be on the ship to open the hold." ); // You must be on the ship to open the hold.
			}
			else if ( m_Boat.IsMoving )
			{
				if ( m_Boat.TillerMan != null )
					m_Boat.TillerMan.Say( true, "I can not open the hold while the ship is moving." ); // I can not open the hold while the ship is moving.
			}
			else
			{
