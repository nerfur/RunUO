	...
	public abstract class BaseBoat : BaseMulti
	{
        ...
        public string StringStatus
        {
            get
            {
                ...
                if (DateTime.Now - start < TimeSpan.FromHours(1.0))
                    return "This structure is like new."; // This structure is like new.

                if (DateTime.Now - start < TimeSpan.FromDays(2.0))
                    return "This structure is slightly worn."; // This structure is slightly worn.

                if (DateTime.Now - start < TimeSpan.FromDays(3.0))
                    return "This structure is somewhat worn."; // This structure is somewhat worn.

                if (DateTime.Now - start < TimeSpan.FromDays(4.0))
                    return "This structure is fairly worn."; // This structure is fairly worn.

                if (DateTime.Now - start < TimeSpan.FromDays(5.0))
                    return "This structure is greatly worn."; // This structure is greatly worn.

                return "This structure is in danger of collapsing."; // This structure is in danger of collapsing.
            }
        }
		...
		public uint CreateKeys( Mobile m )
		{
			...
			if ( !box.TryDropItem( m, bankKey, false ) )
				...
				m.LocalOverheadMessage( MessageType.Regular, 0x3B2, true, "A ship's key is now in my safety deposit box." ); // A ship's key is now in my safety deposit box.

			if ( m.AddToBackpack( packKey ) )
				m.LocalOverheadMessage( MessageType.Regular, 0x3B2, true, "A ship's key is now in my backpack." ); // A ship's key is now in my backpack.
			else
				m.LocalOverheadMessage( MessageType.Regular, 0x3B2, true, "A ship's key is now at my feet." ); // A ship's key is now at my feet.
		...
		public bool LowerAnchor( bool message )
		{
			...
			if ( m_Anchored )
			{
				if ( message && m_TillerMan != null )
					m_TillerMan.Say( true, "Ar, the anchor was already dropped sir." ); // Ar, the anchor was already dropped sir.
			...
			if ( message && m_TillerMan != null )
				m_TillerMan.Say( true, "Ar, anchor dropped sir." ); // Ar, anchor dropped sir.
		...
		public bool RaiseAnchor( bool message )
		{
			...
			{
				if ( message && m_TillerMan != null )
					m_TillerMan.Say( true, "Ar, the anchor has not been dropped sir." ); // Ar, the anchor has not been dropped sir.
			...
			if ( message && m_TillerMan != null )
				m_TillerMan.Say( true, "Ar, anchor raised sir." ); // Ar, anchor raised sir.
		...
		public bool StartMove( Direction dir, bool fast )
		{
			...
			{
				if ( m_TillerMan != null )
					m_TillerMan.Say( true, "Aye aye sir." ); // Aye aye sir.
		...
		public void BeginRename( Mobile from )
		{
			...
			if ( from.AccessLevel < AccessLevel.GameMaster && from != m_Owner )
			{
                if (m_TillerMan != null)
                {
                    ...
                    {
                        case 0: m_TillerMan.Say(true, "Arr, don't do that!"); break;// Arr, don't do that! | Arr, leave me alone! | Arr, watch what thour'rt doing, matey! | Arr! Do that again and I’ll throw ye overhead!
                        case 1: m_TillerMan.Say(true, "Arr, leave me alone!"); break;
                        case 2: m_TillerMan.Say(true, "Arr, watch what thour'rt doing, matey!"); break;
                        case 3: m_TillerMan.Say(true, "Arr! Do that again and I’ll throw ye overhead!"); break;
                    }
                }
				return;
			}
			...
			if ( m_TillerMan != null )
				m_TillerMan.Say( true, "What dost thou wish to name thy ship?" ); // What dost thou wish to name thy ship?
		...
		public void EndRename( Mobile from, string newName )
		{
			...
			{
				if ( m_TillerMan != null )
					m_TillerMan.Say( true, "Arr! Only the owner of the ship may change its name!" ); // Arr! Only the owner of the ship may change its name!
			...
			{
				if ( m_TillerMan != null )
					m_TillerMan.Say( true, "You appear to be dead." ); // You appear to be dead.
		...
		public void Rename( string newName )
		{
			...
			if ( m_TillerMan != null && m_ShipName != null )
				m_TillerMan.Say( true, String.Format( "This ship is now called the {0}", m_ShipName) ); // This ship is now called the ~1_NEW_SHIP_NAME~.
			else if ( m_TillerMan != null )
				m_TillerMan.Say( true, "This ship now has no name." ); // This ship now has no name.
		}
		...
		public void RemoveName( Mobile m )
		{
			...
			{
				if ( m_TillerMan != null )
					m_TillerMan.Say( true, "Ar, this ship has no name." ); // Ar, this ship has no name.
		...
		public void GiveName( Mobile m )
		{
			...
			if ( m_ShipName == null )
				...
				m_TillerMan.Say( true, String.Format("This is the {O}.", m_ShipName )); // This is the ~1_BOAT_NAME~.
		}
		...
		public void GiveNavPoint()
		{
			...
			if ( NextNavPoint < 0 )
				TillerMan.Say( true, "I have no current nav point." ); // I have no current nav point.
			else
				TillerMan.Say( true, String.Format("My current destination navpoint is nav {0}.", (NextNavPoint + 1).ToString()) ); // My current destination navpoint is nav ~1_NAV_POINT_NUM~.
		}
		...
		public void AssociateMap( MapItem map )
		{
			...
			{
				if ( TillerMan != null )
					TillerMan.Say( true, "Ar, that is not a map, tis but a blank piece of paper!" ); // Ar, that is not a map, tis but a blank piece of paper!
			}
			else if ( map.Pins.Count == 0 )
			{
				if ( TillerMan != null )
					TillerMan.Say( true, "Arrrr, this map has no course on it!" ); // Arrrr, this map has no course on it!
			}
			else
			{
				StopMove( false );
				...
				if ( TillerMan != null )
					TillerMan.Say( true, "A map!" ); // A map!
			}
		}
		...
		public bool StartCourse( string navPoint, bool single, bool message )
		{
			...
			if ( number == -1 )
			{
				if ( message && TillerMan != null )
					TillerMan.Say( true, "I don't see that navpoint, sir." ); // I don't see that navpoint, sir.
		...
		public bool StartCourse( bool single, bool message )
		{
			...
			{
				if ( message && TillerMan != null )
					TillerMan.Say( true, "Ar, the anchor is down sir!" ); // Ar, the anchor is down sir!
			...
			{
				if ( message && TillerMan != null )
					TillerMan.Say( true, "I have seen no map, sir." ); // I have seen no map, sir.
			...
			{
				if ( message && TillerMan != null )
					TillerMan.Say( true, "The map is too far away from me, sir." ); // The map is too far away from me, sir.
			...
			if ( message && TillerMan != null )
				TillerMan.Say( true, "Aye aye sir." ); // Aye aye sir.
		...
		public bool StartTurn( int offset, bool message )
		{
			...
			{
				if ( message )
					m_TillerMan.Say( true, "Ar, the anchor is down sir!" ); // Ar, the anchor is down sir!
		...
		public bool Turn( int offset, bool message )
		{
			...
			{
				if ( message )
					m_TillerMan.Say( true, "Ar, can't turn sir." ); // Ar, can't turn sir.
		...
		public bool StopMove( bool message )
		{
			...
			{
				if ( message && m_TillerMan != null )
					m_TillerMan.Say( true, "Er, the ship is not moving sir." ); // Er, the ship is not moving sir.
			...
			else
			{
				...
				if ( maxSpeed == 0 )
				{
					if ( message && this.Order == BoatOrder.Single && TillerMan != null )
						TillerMan.Say( true, String.Format("We have arrived at nav point {0}, sir.", (NextNavPoint + 1).ToString()) ); // We have arrived at nav point ~1_POINT_NUM~ , sir.
					...
					if ( NextNavPoint + 1 < MapItem.Pins.Count )
					{
						...
						{
							if ( message && TillerMan != null )
								TillerMan.Say( true, String.Format("Heading to nav point {0}, sir.", (NextNavPoint + 1).ToString()) ); // Heading to nav point ~1_POINT_NUM~, sir.
					...
					else
					{
						...
						if ( message && this.Order == BoatOrder.Course && TillerMan != null )
							TillerMan.Say( true, "The course is completed, sir." ); // The course is completed, sir.
		...
		public bool Move( Direction dir, int speed, bool message )
		{
			...
			for ( int i = 1; i <= speed; ++i )
			{
				if ( !CanFit( new Point3D( X + (i * rx), Y + (i * ry), Z ), Map, ItemID ) )
				{
					...
					{
						if ( message && m_TillerMan != null )
							m_TillerMan.Say( true, "Ar, we've stopped sir." ); // Ar, we've stopped sir.
