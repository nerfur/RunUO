	...
	public abstract class BaseBoatDeed : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			else
			{
				if ( Core.SE )
					...
					from.LocalOverheadMessage( MessageType.Regular, 0, true, "Where do you wish to place the ship?" ); // Where do you wish to place the ship?
		...
		public void OnPlacement( Mobile from, Point3D p )
		{
			...
			else
			{
				...
				{
                    from.SendAsciiMessage("You may not place a ship while on another ship or inside a house.");
