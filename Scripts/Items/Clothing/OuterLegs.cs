            ...
            else
            {
                if (Effect != ClothEffect.None)
                {
                    if (IsInIDList(from) || from.AccessLevel >= AccessLevel.GameMaster)
                        from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("a skirt {0} ({1} charges)", GetName(), Charges)));
                    else
	...
	public class Kilt : BaseOuterLegs
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                ...
                {
                    if (IsInIDList(from) || from.AccessLevel >= AccessLevel.GameMaster)
                        from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("a kilt {0} ({1} charges)", GetName(), Charges)));
                    else
