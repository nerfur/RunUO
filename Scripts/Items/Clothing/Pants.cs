            ...
            else
            {
                if (Effect != ClothEffect.None)
                {
                    if (IsInIDList(from) || from.AccessLevel >= AccessLevel.GameMaster)
                        from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("short pants {0} ({1} charges)", GetName(), Charges)));
                    else
	...
	public class LongPants : BasePants
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                ...
                {
                    if (IsInIDList(from) || from.AccessLevel >= AccessLevel.GameMaster)
                        from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("long pants {0} ({1} charges)", GetName(), Charges)));
                    else
