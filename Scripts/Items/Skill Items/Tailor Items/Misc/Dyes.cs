...
namespace Server.Items
{
	public class Dyes : Item
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "dyes"));
            }
        }
		...
		{
            from.SendAsciiMessage("Select the dye tub to use the dyes on."); // Select the dye tub to use the dyes on.
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				{
					from.SendAsciiMessage( "Use this on a dye tub." ); // Use this on a dye tub.
				}
			}
		}
	}
