	...
	public class Bandage : Item, IDyable
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (Amount >= 2)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", Amount + " clean bandages"));
                }
                else
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "clean bandages"));
                }
            }
        }
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( from.InRange( GetWorldLocation(), Range ) )
			{
				...
				//from.SendLocalizedMessage( 500948 ); // 
