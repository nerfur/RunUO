	...
	public class FireFieldScroll : SpellScroll
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (Amount >= 2)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", Amount + " Fire Field scrolls"));
                }
                else
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a Fire Field scroll"));
                }
            }
        }
