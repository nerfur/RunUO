...
namespace Server.Items
{
	...
	public class Moongate : Item
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a moongate"));
            }
        }
		...
		{
            if (m != null && m.Tag != null && m.Tag.Contains("nomoongate"))
                return false;
		...
		public virtual void UseGate( Mobile m )
		{
			...
			{
				m.SendAsciiMessage( "You are too busy to do that at the moment." ); // You are too busy to do that at the moment.
			}
		...
		{
			if ( IsInTown( from.Location, from.Map ) && !IsInTown( m_Target, m_TargetMap ) && from.Player || (from.Map != Map.Felucca && TargetMap == Map.Felucca && ShowFeluccaWarning) )
			{
				from.Send( new PlaySound( 0x20E, from.Location ) );

                if (from is PlayerMobile && !((PlayerMobile)from).HasMenu)
                {
                    ((PlayerMobile)from).HasMenu = true;
                    from.SendMenu(new MoongateMenu(from, this));
                }
		...
		public virtual void DelayCallback( Mobile from, int range )
		{
			...
			if ( m_TargetMap != null )
				...
				from.SendAsciiMessage( "This moongate does not seem to go anywhere." );
		}
    ...
    public class MoongateMenu : QuestionMenu
    {
        private Mobile m_From;
        private Moongate m_Gate;
        ...
        public MoongateMenu(Mobile from, Moongate gate ) : base("Dost thou wish to step into the moongate?", m_Options)
        {
            m_From = from;
            m_Gate = gate;
        }
