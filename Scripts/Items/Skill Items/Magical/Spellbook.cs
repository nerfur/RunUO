...
namespace Server.Items
{
	...
	public class Spellbook : Item, ICraftable, ISlayer
	{
		...
		private static void EventSink_CastSpellRequest( CastSpellRequestEventArgs e )
		{
			...
			{
				from.SendAsciiMessage( "You do not have that spell!" ); // You do not have that spell!
			}
		}
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a spellbook"));
            }
        }
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( Parent == from || ( pack != null && Parent == pack ) )
				...
				from.SendAsciiMessage( "The spellbook must be in your backpack (and not in a container within) to open." ); // The spellbook must be in your backpack (and not in a container within) to open.
		}
