...
namespace Server.Items
{
	public abstract class BaseExplosionPotion : BasePotion
	{
		...
		private void Detonate_OnTick( object state )
		{
			...
			{
				if ( parent is Item )
					((Item)parent).PublicOverheadMessage( MessageType.Regular, 0x22, true, timer.ToString() );
				else if ( parent is Mobile )
					((Mobile)parent).PublicOverheadMessage( MessageType.Regular, 0x22, true, timer.ToString() );
