	...
	public class Hides : BaseHides, IScissorable
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (Amount >= 2)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("{0} piles of hides ({0} yards of leather)", Amount)));
                }
                else
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a pile of hides (1 yard of leather)"));
                }
            }
        }
