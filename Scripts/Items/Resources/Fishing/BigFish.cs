...
namespace Server.Items
{
	public class BigFish : Item, ICarvable
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a big fish"));
                if ( m_Fisher != null )
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "Caught by " + m_Fisher.Name));

                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", ((int)Weight).ToString() + " stones"));
            }
        }
