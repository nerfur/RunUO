	...
	public class BlackPearl : BaseReagent, ICommodity
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (Amount >= 2)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", Amount + " Black Pearl"));
                }
                else
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "Black Pearl"));
                }
            }
        }
