	...
	public class Head : Item
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (m_PlayerName == null)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a head"));
                }
                else
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", String.Format("the head of {0}", m_PlayerName)));
                }
            }
        }
