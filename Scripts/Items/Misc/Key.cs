	...
	public class Key : Item
	{
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( m_KeyVal != 0 )
			{
                ...
                from.SendAsciiMessage("What shall I use this key on?");
				t = new UnlockTarget( this );
			}
			else
			{
				//number = 501663; // This key is a key blank. Which key would you like to make a copy of?
                from.SendAsciiMessage("Which key would you like to make a copy of?");
		...
		public override void OnSingleClick( Mobile from )
		{
            ...
            if (this.Name != null)
            {
                ...
                if (desc.Length > 0)
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", desc));
            }
            else
            {
                //Name
                if (this.ItemID == 0x100F)
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a gold key"));
                else if (this.ItemID == 0x1010)
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "an iron key"));
                else if (this.ItemID == 0x100E)
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a copper key"));
                else
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a rusty key"));
				...
				else
				{
					...
					if ( o is Item )
					{
						...
						if ( o.Locked )
                            ...
                            from.Send(new AsciiMessage(item.Serial, item.ItemID, MessageType.Regular, 0, 3, "", "You lock it."));
						else
							//item.SendLocalizedMessageTo( from, 1048001 ); // You unlock it.
                            from.Send(new AsciiMessage(item.Serial, item.ItemID, MessageType.Regular, 0, 3, "", "You unlock it."));

						if ( item is LockableContainer )
						{
							LockableContainer cont = (LockableContainer) item;

                            if (cont.TrapType != TrapType.None && this.IsChildOf(from.Backpack) )
                            {
                                if (o.Locked)
                                {
                                    cont.TrapOnLockpick = false;
                                    //item.SendLocalizedMessageTo( from, 501673 ); // You re-enable the trap.
                                    from.Send(new AsciiMessage(item.Serial, item.ItemID, MessageType.Regular, 0, 3, "", "You re-enable the trap."));
                                }
                                else
                                {
                                    cont.TrapOnLockpick = true;
                                    //item.SendLocalizedMessageTo( from, 501672 ); // You disable the trap temporarily.  Lock it again to re-enable it.
                                    from.Send(new AsciiMessage(item.Serial, item.ItemID, MessageType.Regular, 0, 3, "", "You disable the trap temporarily. Lock it again to re-enable it."));
                                }
                            }
						}
					}
		...
		private class UnlockTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object targeted )
			{
				...
				if ( targeted == m_Key )
				{
                    ...
                    from.SendAsciiMessage("Enter a description for this key.");
                ...
                else if (targeted is HouseSign)
                {
                    ...
                    if (m_Key.KeyValue == sign.keyValue)
                    {
                    ...
                    else
                    {
                        ...
                        from.SendAsciiMessage("This key doesn't seem to work on that.");
                    }
                }
				else
				{
