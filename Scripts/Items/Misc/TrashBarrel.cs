...
namespace Server.Items
{
	public class TrashBarrel : Container, IChopable
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a trash barrel"));
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "(" + this.TotalItems + " items, " + this.TotalWeight + " stones)"));
            }
        }
        ...
        public void Empty(string message)
        {
            ...
            if (items.Count > 0)
            {
                PublicOverheadMessage(Network.MessageType.Regular, 0x3B2, true, message);
		...
		private class EmptyTimer : Timer
		{
			...
			{
				m_Barrel.Empty( "Emptying the trashcan!" ); // Emptying the trashcan!
			}
		}
	}
}