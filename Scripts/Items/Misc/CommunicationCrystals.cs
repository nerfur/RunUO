...
namespace Server.Items
{
	public class CrystalRechargeInfo
	{
		public static readonly CrystalRechargeInfo[] Table = new CrystalRechargeInfo[]
			{
				new CrystalRechargeInfo( typeof( Citrine ), 50 ),
				new CrystalRechargeInfo( typeof( Amber ), 50 ),
				new CrystalRechargeInfo( typeof( Tourmaline ), 75 ),
				new CrystalRechargeInfo( typeof( Emerald ), 100 ),
				new CrystalRechargeInfo( typeof( Sapphire ), 100 ),
				new CrystalRechargeInfo( typeof( Amethyst ), 100 ),
				new CrystalRechargeInfo( typeof( StarSapphire ), 125 ),
				new CrystalRechargeInfo( typeof( Diamond ), 200 )
			};
	...
	{
		public static readonly int MaxCharges = 200;
		...
		public bool Active
		{
            get { return this.ItemID == 0x1ED0; }
			set
			{
                this.ItemID = value ? 0x1ED0 : 0x1ECD;
		...
		[Constructable]
		public BroadcastCrystal() : this( 200 )
		{
		}

		[Constructable]
        public BroadcastCrystal(int charges) : base(0x1ECD)
		{
		...
		public override void OnSingleClick(Mobile from)
		{
            ...
            else
            {
                if (Receivers.Count > 0)
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "an " + (this.Active ? "active" : "inactive") + " crystal of communication with " + this.Charges.ToString() + " charges with " + Receivers.Count.ToString() + " links"));
                else
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "an " + (this.Active ? "active" : "inactive") + " crystal of communication with " + this.Charges.ToString() + " charges"));
            }
        }
		...
		public override void OnSpeech( SpeechEventArgs e )
		{
			...
			foreach ( BroadcastCrystal receiver in new ArrayList( Receivers ) )
			{
				if ( receiver.Deleted )
				{
				...
				{
					receiver.TransmitMessage( from, speech, receiver );
					Charges--;
				}
				else
				{
