...
namespace Server.Items
{
	public class Corpse : Container, ICarvable
	{
		...
		public void TurnToBones()
		{
            ...
            foreach (Item itemm in list)
            {
                itemm.MoveToWorld(Location, Map);
            }
        ...
        public override void DisplayTo(Mobile to)
        {
            ...
            if (to.NetState != null && to.NetState.ContainerGridLines)
                to.Send(new CorpseContent6017(to, this));
            else
                to.Send(new CorpseContent(to, this));
		...
		{
            int hue = 0; //Notoriety.GetHue( NotorietyHandlers.CorpseNotoriety( from, this ) );
			...
			if ( ItemID == 0x2006 ) // Corpse form
			{
				if ( m_CorpseName != null )
                    ...
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, hue, 3, "", "a corpse of " + Name));
			}
			else // Bone form
			{
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, hue, 3, "", "the remains of " + Name));
			}
		}
		...
		public void Carve( Mobile from, Item item )
		{
			...
			else if ( ((Body)Amount).IsHuman && ItemID == 0x2006 )
			{
                ...
                {
                    PlayerMobile pm = dead as PlayerMobile;

                    new Head(pm.Name, pm.Serial).MoveToWorld(Location, Map);
                }
                else
                {
