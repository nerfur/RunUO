	...
	public class OilCloth : Item, IScissorable, IDyable
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (Amount >= 2)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", Amount + " oil cloths"));
                }
                else
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "oil cloth"));
                }
            }
        }
