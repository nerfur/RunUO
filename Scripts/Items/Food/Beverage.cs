...
namespace Server.Items
{
	...
	public class BeverageBottle : BaseBeverage
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                ...
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a bottle"));
                }
                else
                {
                    if (this.Content == BeverageType.Ale)
                    {
                        Drink = " of ale";
                    }
                    else if (this.Content == BeverageType.Cider)
                    {
                        Drink = " of cider";
                    }
                    else if (this.Content == BeverageType.Liquor)
                    {
                        Drink = " of liquor";
                    }
                    else if (this.Content == BeverageType.Milk)
                    {
                        Drink = " of milk";
                    }
                    else if (this.Content == BeverageType.Wine)
                    {
                        Drink = " of wine";
                    }
	...
	public class Jug : BaseBeverage
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (this.IsEmpty == true)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a jug"));
                }
                else
                {
                    if (this.Content == BeverageType.Ale)
                    ...
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", GetQuantityString() + "jug" + Drink + "."));
                }
            }
        }
	...
	public class CeramicMug : BaseBeverage
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (this.IsEmpty == true)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a ceramic mug"));
                }
                else
                {
                    if (this.Content == BeverageType.Ale)
                    ...
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "ceramic mug" + Drink));
                }
            }
        }
	...
	public class PewterMug : BaseBeverage
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (this.IsEmpty == true)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a pewter mug"));
                }
                else
                {
                    if (this.Content == BeverageType.Ale)
                    ...
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "pewter mug" + Drink));
                }
            }
        }
	...
	public class Goblet : BaseBeverage
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (this.IsEmpty == true)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a goblet"));
                }
                else
                {
                    if (this.Content == BeverageType.Ale)
                    ...
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "goblet" + Drink));
                }
            }
        }
	...
	public class GlassMug : BaseBeverage
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (this.IsEmpty == true)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a mug"));
                }
                else
                {
                    if (this.Content == BeverageType.Ale)
                    ...
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "mug" + Drink));
                }
            }
        }
	...
	public class Pitcher : BaseBeverage
	{
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            else
            {
                if (this.IsEmpty == true)
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "glass pitcher"));
                }
                else
                {
                    if (this.Content == BeverageType.Ale)
                    ...
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "pitcher" + Drink));
                }
            }
        }
	...
	public abstract class BaseBeverage : Item, IHasQuantity
	{
        ...
        public virtual string GetQuantityDescriptionString()
        {
            int perc = (m_Quantity * 100) / MaxQuantity;
            string name = "";
        ...
        public virtual string GetQuantityString()
        {
            int perc = (m_Quantity * 100) / MaxQuantity;
            string name = "";
		...
		public virtual bool ValidateUse( Mobile from, bool message )
		{
			...
			if ( !Movable && !Fillable )
			{
				...
				{
					if ( message )
						from.SendAsciiMessage( "That belongs to someone else." ); // That belongs to someone else.
		...
		public virtual void Fill_OnTarget( Mobile from, object targ )
		{
			...
			else if ( targ is Item )
			{
				...
				from.SendAsciiMessage( "You fill the container with water." ); // You fill the container with water.
			}
		...
		public static void CheckHeaveTimer( Mobile from )
		{
			...
			else
			{
				...
				if ( t != null )
				{
					...
					from.SendAsciiMessage( "You feel sober." ); // You feel sober.
				}
			}
		}
			...
			protected override void OnTick()
			{
				...
				else if ( m_Drunk.Alive )
				{
					...
					if ( Utility.Random( 1, 4 ) == 1 )
					{
						...
						// *hic*
						m_Drunk.PublicOverheadMessage( Network.MessageType.Regular, 0x3B2, true, "*hic*" );
					}
					...
					if ( m_Drunk.BAC <= 0 )
					{
						...
						m_Drunk.SendAsciiMessage( "You feel sober." ); // You feel sober.
					}
				}
			}
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			if ( IsEmpty )
			{
                ...
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0, 3, "", "Fill from what?"));
			...
			else if ( Pourable && ValidateUse( from, true ) )
			{
				...
				from.SendAsciiMessage( "What do you want to use this on?" ); // What do you want to use this on?
			}
		}
