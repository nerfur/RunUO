	...
	public class FlourMillEastAddon : BaseAddon, IFlourMill
	{
		...
		private void FinishWorking_Callback( object state )
		{
			...
			if ( from != null && !from.Deleted && !this.Deleted && IsFull )
			{
				...
				{
					flour.Delete();
					from.SendAsciiMessage( "There is not enough room in your backpack!  You stop grinding." ); // There is not enough room in your backpack!  You stop grinding.
				}
			}
		...
		public override void OnComponentUsed( AddonComponent c, Mobile from )
		{
			if ( !from.InRange( GetWorldLocation(), 4 ) || !from.InLOS( this ) )
				...
				from.SendAsciiMessage( "You need more wheat to make a sack of flour." ); // You need more wheat to make a sack of flour.
			else
				StartWorking( from );
		}
		...
		{
            AddComponent(new AddonComponent(0x1920, "a flour mill"), -1, 0, 0);
            AddComponent(new AddonComponent(0x1922, "a flour mill"), 0, 0, 0);
            AddComponent(new AddonComponent(0x1924, "a flour mill"), 1, 0, 0);
		}
