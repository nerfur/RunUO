...
namespace Server.Items
{
	...
	public class TrainingDummy : AddonComponent
	{
		...
		{
			UpdateItemID();
			Effects.PlaySound( GetWorldLocation(), Map, Utility.RandomList( 314,315,316,317/*0x3A4, 0x3A6, 0x3A9, 0x3AE, 0x3B4, 0x3B6*/ ) );
		}
		...
		public override void OnDoubleClick( Mobile from )
		{
			...
			if ( weapon is BaseRanged )
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0, 3, "", "You can't practice ranged weapons on this."));
                ...
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0, 3, "", "You are too far away to do that."));
				//SendLocalizedMessageTo( from, 501816 ); // You are too far away to do that.
			else if ( Swinging )
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0, 3, "", "You have to wait until it stops swinging."));
                ...
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0, 3, "", "Your skill cannot improve any further by simply practicing with a dummy."));
				//SendLocalizedMessageTo( from, 501828 ); // Your skill cannot improve any further by simply practicing with a dummy.
			else if ( from.Mounted )
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Regular, 0, 3, "", "You can't practice on this while on a mount."));
        ...
        public override void OnSingleClick(Mobile from)
        {
            ...
            {
                from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "a training dummy"));
            }
        }
