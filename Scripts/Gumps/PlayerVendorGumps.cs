...
namespace Server.Gumps
{
	public class PlayerVendorBuyGump : Gump
	{
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			{
				m_Vendor.SayTo( from, true, "You own this shop, just take what you want." ); // You own this shop, just take what you want.
				return;
			}
			...
			if ( info.ButtonID == 1 )
			{
				...
				{
					m_Vendor.SayTo( from, true, "You can't buy that." ); // You can't buy that.
					return;
				}
				...
				{
					m_Vendor.SayTo( from, true, "You cannot afford this item." ); // You cannot afford this item.
				}
				else if ( !from.PlaceInBackpack( m_VI.Item ) )
				{
					m_Vendor.SayTo( from, true, "You do not have room in your backpack for this." ); // You do not have room in your backpack for this.
				}
				else
				{
					...
					from.SendAsciiMessage( "You take the item." ); // You take the item.
				}
			}
			else
			{
				from.SendAsciiMessage( "Cancelled purchase." ); // Cancelled purchase.
			}
		}
	}
