	...
	public class FireFieldSpell : MagerySpell
	{
		...
		public void Target( IPoint3D p )
		{
			...
			else if ( SpellHelper.CheckTown( p, Caster ) && CheckSequence() )
			{
                ...
                //duration = TimeSpan.FromSeconds(15 + (Caster.Skills[SkillName.Magery].Value * 0.5));
                int SpellNum = 28;
                duration = TimeSpan.FromSeconds(15 + ((Caster.Skills[SkillName.Magery].Value * 2) / ((SpellNum - 1) / 8) + 1));
		...
		public class FireFieldItem : Item
		{
            ...
            public override void OnSingleClick(Mobile from)
            {
                ...
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "Field of fire"));
                }
            }
