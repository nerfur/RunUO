			...
			else if ( CheckSequence() )
			{
				...
				if ( targets.Count > 0 )
				{
					...
					for ( int i = 0; i < targets.Count; ++i )
					{
						...
						if ( poison != null )
						{
							int chanceToCure = 10000 + (int)(Caster.Skills[SkillName.Magery].Value * 75) - ((poison.Level) * 1750);
							chanceToCure /= 100;
							//chanceToCure -= 1;

                            if (chanceToCure > Utility.Random(100) && m.CurePoison(Caster))
                            {
                                m.SendAsciiMessage(String.Format("{0} has cured you of all poisons!",Caster.Name));
                                ++cured;
                            }
                            else
                                m.SendAsciiMessage(String.Format("{0} has failed to cure you!", Caster.Name));
						}
					...
					//if ( cured > 0 )
						//Caster.SendAsciiMessage( "You have cured the target of all poisons!" ); // You have cured the target of all poisons!
				}
			}
