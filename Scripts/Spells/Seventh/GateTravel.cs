...
namespace Server.Spells.Seventh
{
	public class GateTravelSpell : MagerySpell
	{
		...
		public void Effect( Point3D loc, Map map, bool checkMulti )
		{
			...
			{
				Caster.SendAsciiMessage( "You open a magical gate to another location." ); // You open a magical gate to another location
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object o )
			{
				...
				{
					from.Send( new AsciiMessage( from.Serial, from.Body, MessageType.Regular, 0x3B2, 3,"", " I can not gate travel from that object." ) ); // I can not gate travel from that object.
				}
			}
