	...
	public class PoisonFieldSpell : MagerySpell
	{
		...
		private class InternalItem : Item
		{
            ...
            public override void OnSingleClick(Mobile from)
            {
                ...
                {
                    from.Send(new AsciiMessage(Serial, ItemID, MessageType.Label, 0, 3, "", "Field of Poison"));
                }
            }
            ...
            public void ApplyPoisonTo(Mobile m)
            {
                ...
                if (Utility.Random(1, 100) < m_Caster.Skills[SkillName.Magery].Value)
                    level = 1;
                else
                    level = 3;

                if (m.CheckSkill(SkillName.MagicResist, 0.0, 40.0))
                {
                    m.SendAsciiMessage("You feel yourself resisting magical energy."); // You feel yourself resisting magical energy.
                    return;
                }
                m.ApplyPoison(m_Caster, Poison.GetPoison(level));
            }
