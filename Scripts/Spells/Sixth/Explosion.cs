			...
			protected override void OnTick()
			{
				if ( m_Attacker.HarmfulCheck( m_Defender ) )
				{
					...
					else
					{
                        ...
                        damage = (((6 * Utility.Random(1, 8)) + 8) * (50 + m_Attacker.Skills[SkillName.Magery].Value)) / 100;
						...
						if ( m_Spell.CheckResisted( m_Target, damage ) )
						{
							...
							m_Target.SendAsciiMessage( "You feel yourself resisting magical energy." ); // You feel yourself resisting magical energy.
						}
