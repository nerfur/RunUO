	...
	public class MagicLockSpell : MagerySpell
	{
		...
		public void Target( LockableContainer targ )
		{
			...
			else if ( CheckSequence() )
			{
				...
				Caster.LocalOverheadMessage( MessageType.Regular, 0x3B2, true, "The chest is now locked!" );
		...
		private class InternalTarget : Target
		{
			...
			protected override void OnTarget( Mobile from, object o )
			{
				if ( o is LockableContainer )
					...
					from.SendAsciiMessage( "Target must be an unlocked chest." ); // Target must be an unlocked chest.
			}
