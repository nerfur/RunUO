	...
	public abstract class BaseAI
	{
					...
					switch( m_Order )
					{
						...
						case OrderType.Unfriend:
						{
							if( m_Order == OrderType.Transfer && m_From.HasTrade )
								m_From.SendAsciiMessage( "You cannot transfer a pet with a trade pending." ); // You cannot transfer a pet with a trade pending
							else if( m_Order == OrderType.Friend && m_From.HasTrade )
								m_From.SendAsciiMessage( "You cannot friend a pet with a trade pending." ); // You cannot friend a pet with a trade pending
							else
		...
		public virtual void BeginPickTarget( Mobile from, OrderType order )
		{
			...
			if( from.Target == null )
			{
				if( order == OrderType.Transfer )
					from.SendAsciiMessage( "Click on the person to transfer ownership to." ); // Click on the person to transfer ownership to.
				else if( order == OrderType.Friend )
					from.SendAsciiMessage( "Click on the player whom you wish to make a co-owner." ); // Click on the player whom you wish to make a co-owner.
				else if( order == OrderType.Unfriend )
					from.SendAsciiMessage( "Click on the player whom you wish to remove as a co-owner." ); // Click on the player whom you wish to remove as a co-owner.
		...
		public virtual void EndPickTarget( Mobile from, Mobile target, OrderType order )
		{
			...
			if( order == OrderType.Attack )
			{
                ...
                {
                    if (Notoriety.Compute(m_Mobile, target) == Notoriety.Innocent)//(target.Player || target is BaseVendor || target is BaseGuard || target.Karma > 0)
                    {
                        m_Mobile.Say(true, "I am no murderer!");
                        return;
                    }
				...
				{
					m_Mobile.SayTo( from, true, "Your pet refuses to attack this creature!" );
					return;
				}
				...
				{
					m_Mobile.SayTo( from, true, "Your pet refuses to attack the guard." );
					return;
				}
			}

            //if (!target.Player && m_Mobile is BaseHire)
            //    m_Mobile.Say(true, "Uhh... Sure. If you say so. Uh-huh. No problem. Soon as it gives an order, I'll obey...");
                
			if( m_Mobile.CheckControlChance( from ) )
			{
            //    m_Mobile.Say(true, "Very well, I transfer my allegiance.");
        ...
        public class InteractTimer : Timer
        {
            private BaseCreature m_Mobile;
            private BaseAI m_AI;
            private Mobile m_From;

            public InteractTimer(BaseCreature mobile, BaseAI ai, Mobile from) : base(TimeSpan.FromSeconds(30.0))
            {
                m_Mobile = mobile;
                m_AI = ai;
                m_From = from;
            ...
            protected override void OnTick()
            {
                if (m_Mobile.FocusMob != null)
                {
                    if (m_Mobile.Sophistication == SophisticationLevel.High)
                        m_Mobile.Say(true, BaseSpeech.FarewellHigh(m_Mobile, m_From));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Medium)
                        m_Mobile.Say(true, BaseSpeech.FarewellMedium(m_Mobile, m_From));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Low)
                        m_Mobile.Say(true, BaseSpeech.FarewellLow(m_Mobile, m_From));

                    m_Mobile.FocusMob = null;
                    m_AI.Action = ActionType.Wander;
                }
            }
        }
        ...
        {
            private BaseCreature m_Mobile;
            private SpeechEventArgs e;
            private BaseAI m_AI;

            public ResponseTimer(BaseCreature mobile, SpeechEventArgs p, BaseAI ai) : base(TimeSpan.FromSeconds(1))
            {
                e = p;
                m_Mobile = mobile;
                m_AI = ai;
            ...
            protected override void OnTick()
            {
                ...
                if ((e.HasKeyword(0x003B) || Insensitive.Speech(e.Speech, "hi")) && m_Mobile.FocusMob == null && !e.Handled) //*hello*, *hail*, *greetings*, *hey*, *yo*
                {
                    ...
                    if (m_Mobile.Sophistication == SophisticationLevel.High)
                        m_Mobile.Say(true, BaseSpeech.GreetingHigh(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Medium)
                        m_Mobile.Say(true, BaseSpeech.GreetingMedium(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Low)
                        m_Mobile.Say(true, BaseSpeech.GreetingLow(m_Mobile, e.Mobile));

                    m_AI.ListeningTo(e.Mobile);
                ...
                else if ((e.HasKeyword(0x003B) || Insensitive.Speech(e.Speech, "hi")) && m_Mobile.FocusMob == e.Mobile && !e.Handled)
                {
                    ...
                    if (m_Mobile.Sophistication == SophisticationLevel.High)
                        m_Mobile.Say(true, BaseSpeech.ReGreetingHigh(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Medium)
                        m_Mobile.Say(true, BaseSpeech.ReGreetingMedium(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Low)
                        m_Mobile.Say(true, BaseSpeech.ReGreetingLow(m_Mobile, e.Mobile));

                    m_AI.ListeningTo(e.Mobile);
                }
                //Someone says are name, and we are either with no one or with the same person
                else if (m_AI.WasNamed(e.Speech) && (m_Mobile.FocusMob == null || m_Mobile.FocusMob == e.Mobile) && !e.Handled)
                {
                    e.Handled = true;

                    if (m_Mobile.Sophistication == SophisticationLevel.High)
                        m_Mobile.Say(true, BaseSpeech.ConvoInitHigh(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Medium)
                        m_Mobile.Say(true, BaseSpeech.ConvoInitMedium(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Low)
                        m_Mobile.Say(true, BaseSpeech.ConvoInitLow(m_Mobile, e.Mobile));

                    m_AI.ListeningTo(e.Mobile);
                }
                //We are with someone and they are saying goodbye
                else if (m_Mobile.FocusMob == e.Mobile && !e.Handled && (Insensitive.Speech(e.Speech, "bye") || Insensitive.Speech(e.Speech, "farewell") || Insensitive.Speech(e.Speech, "see ya") || Insensitive.Speech(e.Speech, "cya") || Insensitive.Speech(e.Speech, "see you") || Insensitive.Speech(e.Speech, "hasta")))
                {
                    e.Handled = true;

                    if (m_Mobile.Sophistication == SophisticationLevel.High)
                        m_Mobile.Say(true, BaseSpeech.FarewellHigh(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Medium)
                        m_Mobile.Say(true, BaseSpeech.FarewellMedium(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Low)
                        m_Mobile.Say(true, BaseSpeech.FarewellLow(m_Mobile, e.Mobile));

                    m_Mobile.FocusMob = null;
                    m_AI.Action = ActionType.Wander;

                    if (m_AI.m_InteractTimer != null)
                        m_AI.m_InteractTimer.Stop();
                }
                //We are with someone and they are saying goodbye rudely
                else if (m_Mobile.FocusMob == e.Mobile && !e.Handled && (Insensitive.Speech(e.Speech, "go away") || Insensitive.Speech(e.Speech, "leave me alone") || Insensitive.Speech(e.Speech, "fuck off") || Insensitive.Speech(e.Speech, "take a hike") || Insensitive.Speech(e.Speech, "go fly a kite")))
                {
                    e.Handled = true;

                    if (m_Mobile.Sophistication == SophisticationLevel.High)
                        m_Mobile.Say(true, BaseSpeech.RudeFarewellHigh(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Medium)
                        m_Mobile.Say(true, BaseSpeech.RudeFarewellMedium(m_Mobile, e.Mobile));
                    else if (m_Mobile.Sophistication == SophisticationLevel.Low)
                        m_Mobile.Say(true, BaseSpeech.RudeFarewellLow(m_Mobile, e.Mobile));
                ...
                //We are with someone or no one and they have asked something
                if ((m_Mobile.FocusMob == null || m_Mobile.FocusMob == e.Mobile) && !e.Handled)
                {
                    e.Handled = true;

                    BaseSpeech.GetSpeech(m_Mobile, e);
        ...
        public virtual void ListeningTo(Mobile from)
        {
            if (m_Mobile is BaseHire)
                return;
            ...
            if (m_InteractTimer == null)
                m_InteractTimer = new InteractTimer(m_Mobile, this, from);
            else
		...
		public virtual bool DoActionFlee()
		{
			...
			if( WalkMobileRange( from, 1, false, m_Mobile.RangePerception*2, m_Mobile.RangePerception*3 ) )
			{
			...
			else
			{
				...
				else if( from.CanBeBeneficial( to, true ) )
				{
					...
					if( fromState != null && toState != null )
					{
						if( from.HasTrade )
						{
							from.SendAsciiMessage( "You cannot friend a pet with a trade pending" ); // You cannot friend a pet with a trade pending
						}
						else if( to.HasTrade )
						{
							to.SendAsciiMessage( "You cannot friend a pet with a trade pending" ); // You cannot friend a pet with a trade pending
						}
						else if( m_Mobile.IsPetFriend( to ) )
						{
							from.SendAsciiMessage( "That person is already a friend." ); // That person is already a friend.
						}
						else if( !m_Mobile.AllowNewPetFriend )
						{
							from.SendAsciiMessage( "Your pet does not seem to be interested in making new friends right now." ); // Your pet does not seem to be interested in making new friends right now.
						}
						else
						{
							// ~1_NAME~ will now accept movement commands from ~2_NAME~.
                            from.SendAsciiMessage(String.Format("{0} will now accept movement commands from {1}.", m_Mobile.Name, to.Name));

							/* ~1_NAME~ has granted you the ability to give orders to their pet ~2_PET_NAME~.
							 * This creature will now consider you as a friend.
							 */
                            to.SendAsciiMessage(1043246, String.Format("{0} has granted you the ability to give orders to their pet {1}.", from.Name, m_Mobile.Name));
                            to.SendAsciiMessage("This creature will now consider you as a friend.");
		...
		public virtual bool DoOrderUnfriend()
		{
			...
			if( from == null || to == null || from == to || from.Deleted || to.Deleted || !to.Player )
			{
				m_Mobile.PublicOverheadMessage(MessageType.Regular, 0x3B2, true, "*looks confused*" ); // *looks confused*
			}
			else if( !m_Mobile.IsPetFriend( to ) )
			{
				from.SendAsciiMessage( "That person is not a friend." ); // That person is not a friend.
			}
			else
			{
				// ~1_NAME~ will no longer accept movement commands from ~2_NAME~.
				from.SendAsciiMessage( String.Format( "{0} will no longer accept movement commands from {1}.", m_Mobile.Name, to.Name ) );
				 ...
				 */
				to.SendAsciiMessage( String.Format( "{0} has no longer granted you the ability to give orders to their pet {1}.", from.Name, m_Mobile.Name ) );
                to.SendAsciiMessage("This creature will no longer consider you as a friend.");
		...
		public virtual bool DoOrderTransfer()
		{
			...
			if( from != to && from != null && !from.Deleted && to != null && !to.Deleted && to.Player )
			{
				...
				{
					from.SendAsciiMessage( String.Format("The pet refuses to be transferred because it will not obey {0}.", to.Name) ); // The pet refuses to be transferred because it will not obey ~1_NAME~.~3_BLANK~
                    to.SendAsciiMessage(String.Format("The pet will not accept you as a master because it does not trust you.")); // The pet will not accept you as a master because it does not trust you.~3_BLANK~
				}
				else if( !m_Mobile.CanBeControlledBy( from ) )
				{
                    from.SendAsciiMessage(String.Format("The pet refuses to be transferred because it will not obey you sufficiently."));
                    to.SendAsciiMessage(String.Format("The pet will not accept you as a master because it does not trust {0}.",from.Name));

				}
				else if( TransferItem.IsInCombat( m_Mobile ) )
				{
					from.SendAsciiMessage( "You may not transfer a pet that has recently been in combat." );
					to.SendAsciiMessage( "The pet may not be transfered to you because it has recently been in combat." );
				}
				else
				{
                    if (m_Mobile == null || m_Mobile.Deleted || m_Mobile.ControlMaster != from || !from.CheckAlive() || !to.CheckAlive())
                        return false;

                    if (from.Map != m_Mobile.Map || !from.InRange(m_Mobile, 14))
                        return false;
                    ...
                    if (m_Mobile.SetControlMaster(to))
                    {
                        ...
                        from.SendAsciiMessage(String.Format("You have transferred your pet to {0}.",to.Name)); // You have transferred your pet to ~3_GETTER~.
                        to.SendAsciiMessage(String.Format("{0} has transferred the allegiance of {1} to you.", from.Name, m_Mobile.Name)); // ~1_NAME~ has transferred the allegiance of ~2_PET_NAME~ to you.
