	...
	public class DefInscription : CraftSystem
	{
		...
		public override int CanCraft( Mobile from, BaseTool tool, Type typeItem )
		{
			...
			if ( typeItem != null )
			{
				...
				if ( o is SpellScroll ) 
				{
					...
					scroll.Delete();
                    if (!hasSpell)
                        from.SendAsciiMessage("You don't have that spell!");
		...
		public override int PlayEndingEffect( Mobile from, bool failed, bool lostMaterial, bool toolBroken, int quality, bool makersMark, CraftItem item )
		{
			...
			else
			{
                if (failed)
                {
                    from.SendAsciiMessage("You fail to inscribe the scroll, and the scroll is ruined.");
                    return 0; // You fail to inscribe the scroll, and the scroll is ruined.
                }
                else
                {
                    from.SendAsciiMessage("You inscribe the spell and put the scroll in your backpack.");
                    return 0; // You inscribe the spell and put the scroll in your backpack.
                }
			}
		}
