...
namespace Server.Engines.Craft
{
	...
	public class CraftItem
	{
		...
		public bool ConsumeAttributes( Mobile from, ref object message, bool consume )
		{
			...
			{
                from.LocalOverheadMessage(MessageType.Regular, 0x22, true, "Insufficient mana for this spell.");
                //from.SendAsciiMessage("You lack the required mana to make that.");
		...
		public void Craft( Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool )
		{
							...
							if( badCraft <= 0 )
							{
								...
								if( ConsumeRes( from, typeRes, craftSystem, ref resHue, ref maxAmount, ConsumeType.None, ref message ) )
								{
									...
									if( ConsumeAttributes( from, ref message, false ) )
									{
										...
										iRandom += iMin + 1;
                                        if (craftSystem is DefBowFletching && (ItemType == typeof(Arrow) || ItemType == typeof(Bolt)))
                                            iRandom = 1;
			...
			{
				from.SendAsciiMessage( "You must wait to perform another action." ); // You must wait to perform another action
			}
		}
        ...
        public void CompleteCraft(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CustomCraft customCraft)
        {
            ...
            CompleteCraft(quality, makersMark, from, craftSystem, typeRes, tool, customCraft, p);
        }
		...
		private class InternalTimer : Timer
		{
			...
			protected override void OnTick()
			{
                ...
                if (m_CraftSystem is DefAlchemy)
                {
                    ...
                    item = null;
                    string name;
                    try { item = Activator.CreateInstance(m_TypeRes) as Item; }
                    catch { }
                    name = item.GetType().Name;
                    name = name.Replace("sS", "s S");
                    name = name.Replace("kP", "k P");
                    name = name.Replace("eR", "e R");
                    name = name.Replace("tS", "t S");
                    name = name.Replace("sA", "s A");
                    //name = name.ToLower();
                    ...
                    if (m_iCount == 1)
                    {
                        m_From.LocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("*You start grinding some {0} in the mortar.*", name));
                        m_From.NonlocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("*{0} starts grinding some {1} in a mortar.*", m_From.Name, name));
                        /*int OldHue = m_From.SpeechHue;
                        m_From.SpeechHue = 0x21;
                        m_From.Say(true, String.Format("*{0} starts grinding some {1} in a mortar*", m_From.Name, name));
                        m_From.SpeechHue = OldHue;*/
                    }
                    else if (m_iCount < m_iCountMax)
                    {
                        m_From.LocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("*You continue grinding.*"));
                        m_From.NonlocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("*{0} continues grinding.*", m_From.Name));
                    }
                ...
                {
                    if (m_CraftItem.ItemType == typeof(Arrow) || m_CraftItem.ItemType == typeof(Bolt))
                    {
                        m_From.PlaySound(79);
                    ...
                    else
                    {
                        if (m_iCount == 1)
                        {
                            Item item = null;
                            string name;
                            try { item = Activator.CreateInstance(m_CraftItem.ItemType) as Item; }
                            catch { }
                            ...
                            m_From.LocalOverheadMessage(MessageType.Regular, m_From.EmoteHue, true, String.Format("*You start carving {0}.*", name));
                            m_From.NonlocalOverheadMessage(MessageType.Regular, m_From.EmoteHue, true, String.Format("*{0} starts carving {1}.*", m_From.Name, name));
                            /*int OldHue = m_From.SpeechHue;
                            m_From.SpeechHue = m_From.EmoteHue;
                            m_From.Say(true, String.Format("*begins carving*", name));
                            m_From.SpeechHue = OldHue;*/
                        }
                    }
                }
				...
				else
				{
                    ...
                    if (quality == 2 && m_From.Skills[m_CraftSystem.MainSkill].Base >= 100.0)
                        makersMark = m_CraftItem.IsMarkable(m_CraftItem.ItemType) && !(m_CraftSystem is DefBlacksmithy);
                    else if (m_CraftSystem is DefTailoring && m_From.Skills[m_CraftSystem.MainSkill].Base >= 100.0)
                        makersMark = true;
					...
					else
					{
                        ...
                        {
                            m_From.Target = new InternalTarget(quality, makersMark, m_From, m_CraftSystem, m_TypeRes, m_Tool, null, m_CraftItem);
                            m_From.SendAsciiMessage("Put it where?");
                        }
                        else
        ...
        private class InternalTarget : Target
        {
            ...
            public InternalTarget(int quality, bool makersMark, Mobile from, CraftSystem craftSystem, Type typeRes, BaseTool tool, CustomCraft customCraft, CraftItem CraftItem) : base(3, true, TargetFlags.None)
            {
                CheckLOS = true;
                m_Quality = quality;
                m_MakersMark= makersMark;
                m_From = from;
                m_CraftSystem = craftSystem;
                m_TypeRes = typeRes;
                m_Tool = tool;
            ...
            protected override void OnTarget(Mobile from, object o)
            {
                ...
                if (ip != null)
                {
                    ...
                    if ((TileData.LandTable[landTile.ID & 0x3FFF].Flags & TileFlag.Impassable) != 0)
                        from.SendAsciiMessage("You cannot create that there.");
                    else if (!map.CanFit(p.X, p.Y, p.Z, item.ItemData.Height, false, false, true))
                        from.SendAsciiMessage("You cannot create that there.");
                    else
                        m_CraftItem.CompleteCraft(m_Quality, m_MakersMark, m_From, m_CraftSystem, m_TypeRes, m_Tool, null, p);
