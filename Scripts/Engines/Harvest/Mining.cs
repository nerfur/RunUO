...
namespace Server.Engines.Harvest
{
	public class Mining : HarvestSystem
	{
		...
		private Mining()
		{
			...
			oreAndStone.NoResourcesMessage = "There is no metal here to mine.";
			oreAndStone.DoubleHarvestMessage = "Someone has gotten to the metal before you.";
			oreAndStone.TimedOutOfRangeMessage = "You have moved too far away to continue mining.";
			oreAndStone.OutOfRangeMessage = "That is too far away.";
			oreAndStone.FailMessage = "You loosen some rocks but fail to find any useable ore.";
            oreAndStone.PackFullMessage = "You dig some ore but you have no place to put it.";//"Your backpack is full, so the ore you mined is lost.";
			oreAndStone.ToolBrokeMessage = "You have worn out your tool!";
		...
		public override bool CheckHarvest( Mobile from, Item tool )
		{
			...
			if ( from.Mounted )
			{
                from.SendAsciiMessage("You can't mine while riding.");
				//from.SendLocalizedMessage( 501864 ); // You can't mine while riding.
				return false;
			}
			else if ( from.IsBodyMod && !from.Body.IsHuman )
			{
                from.SendAsciiMessage("You can't mine while polymorphed.");
		...
		public override void SendSuccessTo( Mobile from, Item item, HarvestResource resource )
		{
            ...
            else if (item is IronOre || item is SmallIronOre || item is SmallestIronOre)
                from.SendAsciiMessage("You dig some ore and put it in your backpack.");
            else
		...
		public override bool BeginHarvesting( Mobile from, Item tool )
		{
            ...
            from.SendAsciiMessage("Where do you wish to dig?");
		...
		public override void OnBadHarvestTarget( Mobile from, Item tool, object toHarvest )
		{
			if ( toHarvest is LandTarget )
                from.SendAsciiMessage("You can't mine there.");
				//from.SendLocalizedMessage( 501862 ); // You can't mine there.
			else
                from.SendAsciiMessage("You can't mine that.");
