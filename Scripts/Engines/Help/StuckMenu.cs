	...
	public class StuckMenu : Gump
	{
		...
		public StuckMenu( Mobile beholder, Mobile beheld, bool markUse ) : base( 150, 50 )
		{
			...
			for ( int i = 0; i < 1; i++ )
			{
				...
				AddHtml( 75, 55 + 35 * i, 335, 40, /*entry.Name*/"Random Mainland City", false, false );
			}
		...
		public override void OnResponse( NetState state, RelayInfo info )
		{
			...
			else if ( info.ButtonID == 0 )
			{
				if ( m_Mobile == m_Sender )
					m_Mobile.SendAsciiMessage( "You choose not to go to any city." ); // You choose not to go to any city.
			}
			else
			{
				...
				if ( index >= 0 && index < entries.Length )
                    Teleport(entries[Utility.RandomMinMax(0, entries.Length-1)]);
			}
		}
		...
		private void Teleport( StuckMenuEntry entry )
		{
			if ( m_MarkUse ) 
			{
				m_Mobile.SendAsciiMessage( "You will be teleported within the next two minutes." ); // You will be teleported within the next two minutes.
