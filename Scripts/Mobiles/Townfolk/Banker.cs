				...
				for ( int i = 0; i < e.Keywords.Length; ++i )
				{
					...
					switch ( keyword )
					{
						case 0x0000: // *withdraw*
						{
							...
							{
								this.Say( true, "I will not do business with a criminal!" ); // I will not do business with a criminal!
								break;
							}
							...
							if ( split.Length >= 2 )
							{
								...
								if ( amount > 5000 )
								{
                                    this.Say(true, "Thou canst not withdraw so much at one time!"); // Thou canst not withdraw so much at one time!
								}
								else if ( amount > 0 )
								{
									...
									{
                                        this.Say(true, "Ah, art thou trying to fool me? Thou hast not so much gold!"); // Ah, art thou trying to fool me? Thou hast not so much gold!
									}
									else
									{
										e.Mobile.AddToBackpack( new Gold( amount ) );

                                        this.Say(true, "Thou hast withdrawn gold from thy account."); // Thou hast withdrawn gold from thy account.
									}
								}
							}
						...
						case 0x0001: // *balance*
						{
							...
							if ( box != null )
								this.Say( true, String.Format("Thy current bank balance is {0}", box.TotalGold.ToString()) ); // Thy current bank balance is ~1_AMOUNT~ gold.
							else
								this.Say( true, "They current bank balance is 0 gold." ); // Thy current bank balance is ~1_AMOUNT~ gold.
						...
						case 0x0002: // *bank*
						{
							...
							if ( e.Mobile.Criminal )
							{
								this.Say( true, "Thou art a criminal and cannot access thy bank box." ); // Thou art a criminal and cannot access thy bank box.
								break;
							}
