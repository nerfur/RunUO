...
namespace Server.Mobiles
{
	...
	public class Slime : BaseCreature
	{
        ...
        {
            if ((damamt > (this.Hits / 4)) && (this.Hits > 5))
            {
                Say(true, "*The slime splits when struck!*");
                BaseCreature slime = new Slime();
                slime.Hits = (this.Hits/2);
                this.Hits /= 2;
                slime.MoveToWorld(new Point3D(this.X, this.Y, this.Z), this.Map);
            }
