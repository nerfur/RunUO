...
namespace Server.Mobiles
{
			...
			if ( RootParent is PlayerVendor )
			{
				...
				if ( vi != null )
				{
                    if (!String.IsNullOrEmpty(vi.Description))
                    {
                        ...
                        from.Send(new AsciiMessage(item.Serial, item.ItemID, MessageType.Label, 0, 3, "", vi.Description));
                        //item.LabelTo(from, "Description: {0}", vi.Description);
                    }

					if ( !vi.IsForSale )
                        from.Send(new AsciiMessage(item.Serial, item.ItemID, MessageType.Label, 0, 3, "", "Price: Not for sale."));
						//item.LabelTo( from, 1043307 ); // Price: Not for sale.
					else if ( vi.IsForFree )
                        from.Send(new AsciiMessage(item.Serial, item.ItemID, MessageType.Label, 0, 3, "", "Price: FREE!"));
						//item.LabelTo( from, 1043306 ); // Price: FREE!
					else
                        from.Send(new AsciiMessage(item.Serial, item.ItemID, MessageType.Label, 0, 3, "", String.Format("Price: {0}", vi.Price.ToString())));
	...
	public class PlayerVendor : Mobile
	{
		...
		public override bool OnDragDrop( Mobile from, Item item )
		{
			if ( !IsOwner( from ) )
			{
				SayTo( from, true, "I can only take item from the shop owner." ); // I can only take item from the shop owner.
				return false;
			}
			...
			if ( item is Gold )
			{
				if ( BaseHouse.NewVendorSystem )
				{
					...
					{
						SayTo( from, true, "I'll take that to fund my services." ); // I'll take that to fund my services.
					...
					{
						from.SendAsciiMessage( "Your vendor has sufficient funds for operation and cannot accept this gold." ); // Your vendor has sufficient funds for operation and cannot accept this gold.
			...
			else
			{
				...
				{
					SayTo( from, true, "I can't carry any more." ); // I can't carry any more.
					return false;
				}
			}
		}
		...
		private void OnItemGiven( Mobile from, Item item )
		{
			...
			if ( vi != null )
			{
				...
				from.SendAsciiMessage( String.Format("Type in a price and description for this item. (ESC=not for sale)", name) ); // Type in a price and description for ~1_ITEM~ (ESC=not for sale)
		...
		public override bool CheckNonlocalLift( Mobile from, Item item )
		{
			if ( item.IsChildOf( this.Backpack ) )
			{
				...
				{
					SayTo( from, true, "If you'd like to purchase an item, just ask." ); // If you'd like to purchase an item, just ask.
					return false;
				}
			}
		...
		public void OpenBackpack( Mobile from )
		{
			if ( this.Backpack != null )
			{
				SayTo( from, true, IsOwner( from ) ? "Take a look at your goods." : "Take a look at my goods." ); // Take a look at my/your goods.
		...
		public static void TryToBuy( Item item, Mobile from )
		{
			...
			{
				vendor.SayTo( from, true, "You own this shop, just take what you want." ); // You own this shop, just take what you want.
				return;
			}
			...
			if ( vi == null )
			{
				vendor.SayTo( from, true, "You can't buy that." ); // You can't buy that.
			}
			else if ( !vi.IsForSale )
			{
				vendor.SayTo( from, true, "This item is not for sale." ); // This item is not for sale.
			}
			else if ( vi.Created + TimeSpan.FromMinutes( 1.0 ) > DateTime.Now )
			{
				from.SendAsciiMessage( "You cannot buy this item right now.  Please wait one minute and try again." );
			}
			else
			{
		...
		public void CollectGold( Mobile to )
		{
			if ( HoldGold > 0 )
			{
				SayTo( to, true, "How much of the {0} that I'm holding would you like?", HoldGold.ToString() );
				to.SendAsciiMessage( "Enter the amount of gold you wish to withdraw (ESC = CANCEL):" );

				to.Prompt = new CollectGoldPrompt( this );
			}
			else
			{
				SayTo( to, true, "I am holding no gold for you." ); // I am holding no gold for you.
			}
		}
		...
		public int GiveGold( Mobile to, int amount )
		{
			...
			if ( amount > HoldGold )
			{
				SayTo( to, true, "I'm sorry, but I'm only holding {0} gold for you.", HoldGold.ToString() );
				return 0;
			}
			...
			if ( amountGiven > 0 )
			{
				to.SendAsciiMessage( String.Format("{0} gold has been deposited into your bank box.",amountGiven.ToString()) ); // ~1_AMOUNT~ gold has been deposited into your bank box.
			}

			if ( amountGiven == 0 )
			{
				SayTo( to, true, "Your bank box cannot hold the gold you are requesting.  I will keep the gold until you can take it." ); // Your bank box cannot hold the gold you are requesting.  I will keep the gold until you can take it.
			}
			else if ( amount > amountGiven )
			{
				SayTo( to, true, "I can only give you part of the gold now, as your bank box is too full to hold the full amount." ); // I can only give you part of the gold now, as your bank box is too full to hold the full amount.
			}
			else if ( HoldGold > 0 )
			{
				SayTo( to, true, "Your gold has been transferred." ); // Your gold has been transferred.
			}
			else
			{
				SayTo( to, true, "All the gold I have been carrying for you has been deposited into your bank account." ); // All the gold I have been carrying for you has been deposited into your bank account.
			}
		...
		public void Dismiss( Mobile from )
		{
			...
			{
				SayTo( from, true, "You cannot dismiss me while I am holding your goods." ); // You cannot dismiss me while I am holding your goods.
				return;
			}
				...
				if ( price < 0 ) // Not for sale
				{
					...
					if ( item is Container )
					{
						if ( item is LockableContainer && ((LockableContainer)item).Locked )
							m_Vendor.SayTo( from, true, "Locked items may not be made not-for-sale." ); // Locked items may not be made not-for-sale.
						else if ( item.Items.Count > 0 )
							m_Vendor.SayTo( from, true, "To be not for sale, all items in a container must be for sale." ); // To be not for sale, all items in a container must be for sale.
						else
							setPrice = true;
					}
					else if ( item is BaseBook || item is Engines.BulkOrders.BulkOrderBook || item is KeyRing )
					{
						setPrice = true;
					}
					else
					{
						m_Vendor.SayTo( from, true, "Only the following may be made not-for-sale: books, containers, keyrings, and items in for-sale containers." ); // Only the following may be made not-for-sale: books, containers, keyrings, and items in for-sale containers.
					}
				}
				else
				{
					...
					{
						price = 100000000;
						from.SendAsciiMessage( "You cannot price items above 100,000,000 gold.  The price has been adjusted." );
					}
		...
		private class CollectGoldPrompt : Prompt
		{
			...
			private void GiveGold( Mobile to, int amount )
			{
				if ( amount <= 0 )
				{
					m_Vendor.SayTo( to, true, "Very well. I will hold on to the money for now then." );
				}
				else
				{
