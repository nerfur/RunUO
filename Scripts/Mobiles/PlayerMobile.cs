...
namespace Server.Mobiles
{
	...
	public class PlayerMobile : Mobile, IHonorTarget
	{
		...
		*/
        ...
        private bool m_HasMenu;
        [CommandProperty(AccessLevel.GameMaster)]
        public bool HasMenu
        {
            get { return m_HasMenu; }
            set { m_HasMenu = value; }
        }
        ...
        private bool m_AssumePlayAsGhost;
        [CommandProperty(AccessLevel.GameMaster)]
        public bool AssumePlayAsGhost
        {
            get { return m_AssumePlayAsGhost; }
            set { m_AssumePlayAsGhost = value; }
        }
        ...
        private bool m_BountyMark;
        [CommandProperty(AccessLevel.GameMaster)]
        public bool BountyMark
        {
            get { return m_BountyMark; }
            set { m_BountyMark = value; }
        }
        ...
        public class CheckSkillTimer : Timer
        {
            private Mobile from;

            public CheckSkillTimer(Mobile m) : base(TimeSpan.FromSeconds(1), TimeSpan.FromSeconds(1))
            {
            ...
            {
                //debuging purposes
                //World.Broadcast(0x35, true, String.Format("{0}", from.Name));

                if ((from.Karma >= 80) || (from.Karma <= -80))
                    from.Fame = 10000;
                else
                    from.Fame = 0;
                ...
                {
                    if ((from.Skills[i].Lock == SkillLock.Down) || (from.Skills[i].Lock == SkillLock.Locked))
                    {
                        from.Skills[i].SetLockNoRelay(SkillLock.Up); from.Skills[i].Update();
                        from.SendAsciiMessage("You cannot set your skills DOWN or LOCKED. This has been disabled for historical accuracy.");
                    }
                }
                ...
                if (from.StrLock != StatLockType.Up)
                {
                    ...
                    from.SendAsciiMessage("You cannot set your stats DOWN or LOCKED. This has been disabled for historical accuracy.");
                }
        ...
        public override int MaxWeight { get { return (((Core.ML && this.Race == Race.Human) ? 100 : 70) + (int)(4.0 * (this.Str - 10))); } }
        ...
        public static void CompanionMessage(int hue, string message)
        {
            foreach (NetState state in NetState.Instances)
            {
                ...
                if (pm != null && pm.Companion)
                    pm.SendAsciiMessage(hue, message);
            }
        }
		...
		private static void OnLogin( LoginEventArgs e )
		{
            ...
            Tips.Add("There are no damage modifiers in this era (except the weapon skill itself). Anatomy, evaluating intelligence, lumberjacking, etc, do not add extra damage in combat.");
            Tips.Add("Meditation does not exist in PreT2A. Mana regenerates at a fixed rate for everyone. Armor does not affect regeneration rates, so someone wearing a full plate armor suit will regenerate mana as fast as someone without armor.");

            int count = 1;
            foreach (string message in Tips)
            {
                from.Send(new ScrollMessage(0x00, count, message));
                count++;
            }*/
            char mynew = (char)0xA;
            ...
            if (from is PlayerMobile)
            {
                Account accoun = (Account)from.Account;
                String tag = accoun.GetTag("Beta");

                for (int i = 0; i < BetaAccounts.Length; ++i)
                {
                    if (tag == null || tag == "")
                    {
                        if (accoun.Username == BetaAccounts[i])
                            accoun.AddTag("Beta", "true");
                    }
                }

                tag = accoun.GetTag("Beta");

                if (tag == "true")
                {
                    if (from.Backpack != null)
                    {
                        Item item = new BetaCandle();

                        from.Backpack.DropItem(item);
                        from.SendAsciiMessage("For being a part of the Beta Phase, you have recieved a reward for your efforts.");
                        accoun.SetTag("Beta", "false");
                    }
                }
            }
            ...
            if (from is PlayerMobile)
            {
                ...
                if (myAccount != null)
                {
                    ...
                    {
                        /*if (myAccount.Created >= DateTime.Now-TimeSpan.FromDays(1.0))
                            CompanionMessage(0x49, String.Format("[Companion] A NEW ACCOUNT has logged in. (Player: {0})", from.Name));
                        else if (myAccount.TotalGameTime <= TimeSpan.FromHours(20.0))
                            CompanionMessage(0x49, String.Format("[Companion] A NEW PLAYER has logged in: (Player: {0})", from.Name));
                        else if (DateTime.Now-myAccount.LastLogin >= TimeSpan.FromDays(30.0))
                            CompanionMessage(0x49, String.Format("[Companion] An INACTIVE PLAYER has logged in: (Player: {0})", from.Name));*/
                        if (myAccount.TotalGameTime <= TimeSpan.FromHours(20.0))
                            CompanionMessage(0x49, String.Format("[Companion Program] A new player has logged in. (Player: {0})", from.Name));
                    }
                }
            }
		...
		private static void OnLogout( LogoutEventArgs e )
		{
            if (e.Mobile is PlayerMobile)
            {
                ...
                if (((PlayerMobile)e.Mobile).Companion)
                    ((PlayerMobile)e.Mobile).CompanionLastLocation = Point3D.Zero;
            }
		}
		...
		private static void EventSink_Connected( ConnectedEventArgs e )
		{
			...
			if ( pm != null )
			{
                ...
                pm.m_CheckTimer = new CheckSkillTimer(pm);
                pm.m_CheckTimer.Start();

                pm.m_SwingTimer = new SwingCounterTimer(m);
                pm.m_SwingTimer.Start();
		...
		private static void EventSink_Disconnected( DisconnectedEventArgs e )
		{
			...
			if ( pm != null )
			{
                ...
                pm.m_CheckTimer.Stop();
                pm.m_SwingTimer.Stop();
		...
		public override void OnSubItemAdded( Item item )
		{
			if ( AccessLevel < AccessLevel.GameMaster && item.IsChildOf( this.Backpack ) )
			{
                ...
                if (curWeight > maxWeight)
                    this.SendAsciiMessage(String.Format("You are overloaded. You can carry {0} stones, but are carrying {1} stones.", maxWeight, curWeight));
        ...
        {
            GuardedRegion town = this.Region as GuardedRegion;
            ...
            if (IsPet(target as BaseCreature))
            {
                ...
                if (master != null && master.Karma < -39 && master.Karma > -110 && target.Combatant != null && target.Combatant == this)
                {
                    if (town != null && !town.IsDisabled())
                        return;
                }
            }
            else
            {
                if (target.Karma < -39 && target.Karma > -110 && target.Combatant != null && target.Combatant == this)
                {
                    if (town != null && !town.IsDisabled())
                        return;
                }
		...
		{
            return false;// (m_DesignContext == null);
		}
		...
		public virtual void RecheckTownProtection()
		{
			...
			if ( isProtected != m_LastProtectedMessage )
			{
				if ( isProtected )
                    ...
                    SendAsciiMessage("You are now under the protection of Lord British's guards.");
				else
					//SendLocalizedMessage( 500113 ); // You have left the protection of the town guards.
                    SendAsciiMessage("You have left the protection of Lord British's guards.");
		...
		public override bool CheckEquip( Item item )
		{
			...
			if ( this.AccessLevel < AccessLevel.GameMaster && item.Layer != Layer.Mount && this.HasTrade )
			{
				...
				SendAsciiMessage( "You can only equip what you are already carrying while you have a trade pending." ); // You can only equip what you are already carrying while you have a trade pending.
				return false;
			}
		...
		public override bool CheckNonlocalDrop( Mobile from, Item item, Item target )
		{
			...
			if ( from == this && this.HasTrade && ( target == pack || target.IsChildOf( pack ) ) )
			{
				...
				SendAsciiMessage( "You can't do that while you have a trade pending." ); // You can't do that while you have a trade pending.
				return false;
			}
        ...
        private bool IsWrongWall(Item item)
        {
            if (item is WrongMazeWallNorth || item is WrongMazeWallEast || item is WrongMazeArchNorth1 || item is WrongMazeArchNorth2 || item is WrongMazeArchNorth3 || item is WrongMazeArchEast1 || item is WrongMazeArchEast2 || item is WrongMazeArchEast3)
                return true;
            else
                return false;
        }
		...
		{
            /*if (Location.X > 5684 && Location.Y > 621 && Location.X <= 5684 + 39 && Location.Y <= 621 + 48)
            {
                IPooledEnumerable eable = GetItemsInRange(1);

                foreach (object o in eable)
                {
                    if (o is Item && IsWrongWall(o as Item) && InRange(((Item)o).Location, 1))
                    {
                        ((Item)o).Visible = true;
                    }
                    else if (o is Item && IsWrongWall(o as Item))
                    {
		...
		public override bool OnBeforeDeath()
		{
            ...
            //Order Shield
            if (FindItemOnLayer(Layer.TwoHanded) is OrderShield)
            {
                FindItemOnLayer(Layer.TwoHanded).Delete();
            }
            if (Backpack != null && Backpack.FindItemByType(typeof(OrderShield)) != null)
            {
                Backpack.FindItemByType(typeof(OrderShield)).Delete();
            }

            //Chaos Shield
            if (FindItemOnLayer(Layer.TwoHanded) is ChaosShield)
            {
                FindItemOnLayer(Layer.TwoHanded).Delete();
            }
            if (Backpack != null && Backpack.FindItemByType(typeof(ChaosShield)) != null)
            {
                Backpack.FindItemByType(typeof(ChaosShield)).Delete();
            }
		...
		public override void OnDeath( Container c )
		{
            ...
            if (!AssumePlayAsGhost)
            {
                CantWalk = true;
                new ResNowOption.ResNowOptionTimer(this).Start();
            }
            ...
            if (Bounty >= 5000)
            {
                loss = 1 - (((double)Bounty / 10000) / 100);
                loss -= (Utility.Random(9) / 1000);

                if (RawStr * loss > 10)
                    RawStr = (int)(RawStr * loss);
                if (RawInt * loss > 10)
                    RawInt = (int)(RawInt * loss);
                if (RawDex * loss > 10)
                    RawDex = (int)(RawDex * loss);

                for (int s = 0; s < Skills.Length; s++)
                {
                    if (Skills[s].Base * loss > 35)
                        Skills[s].Base *= loss;
                }
            }*/
            ...
            if (BountyMark)
            {
                ...
                {
                    PlayerMobile killr = killer as PlayerMobile;

                    //do they have more kills than you and are they more than neutral notoriety
                    if ((killr.Kills < this.Kills) && (killr.Karma >= 0))
                    {
                        /*double loss = 0.90; // 10% loss
                        ...
                        //give killer head of the murderer
                        if (killr.Backpack != null)
                        {
                            killr.Backpack.DropItem(new Head(HeadType.Regular, this.Name, this.Serial, killr, DateTime.Now));
                            killr.SendAsciiMessage("As you kill them, you realize there is a bounty on their head! You take the head as evidence of your victory.");
                        }
                        BountyMark = false;
                    }
                }
        ...
        private DateTime m_LastKarmaGain;
        private Timer m_CheckTimer;
        private Timer m_SwingTimer;
		private DateTime m_NextSmithBulkOrder;
		private DateTime m_NextTailorBulkOrder;
		private DateTime m_SavagePaintExpiration;
        private TimeSpan m_DeathCountElapse;
        ...
        public DateTime LastKarmaGain
        {
            get { return m_LastKarmaGain; }
            set { m_LastKarmaGain = value; }
        }
        ...
        public override void OnAfterResurrect()
        {
            ...
            if (ShortTermMurders > 5)
                ...
                scale = (ShortTermMurders*2);

            scale /= 100;
            scale = (1 - scale);

            Server.Skills skill = Skills;
            for (int i = 0; i < skill.Length; ++i)
                skill[i].Base = (skill[i].Base * scale);

            Str = (int)((double)Str * scale);
            Dex = (int)((double)Dex * scale);
            Int = (int)((double)Int * scale);*/
        ...
        public override void OnPoisoned(Mobile from, Poison poison, Poison oldPoison)
        {
            if (poison != null)
            {
                switch ((poison.Level * 2))
                {
                    case 0: this.LocalOverheadMessage(MessageType.Regular, 0x21, true, "*You feel a bit nauseous*"); break;
                    case 2: this.LocalOverheadMessage(MessageType.Regular, 0x21, true, "* You feel disoriented and nauseous! *"); break;
                    case 4: this.LocalOverheadMessage(MessageType.Regular, 0x21, true, "* You begin to feel pain throughout your body! *"); break;
                    case 6: this.LocalOverheadMessage(MessageType.Regular, 0x21, true, "* You feel extremely weak and are in severe pain! *"); break;
                    case 8: this.LocalOverheadMessage(MessageType.Regular, 0x21, true, "* You are in extreme pain, and require immediate aid! *"); break;
                }
                //this.LocalOverheadMessage(MessageType.Regular, 0x21, true, 1042857 + (poison.Level * 2));
                switch ((poison.Level * 2))
                {
                    case 0: this.NonlocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("*{0} looks ill.*", this.Name)); break;
                    case 2: this.NonlocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("* {0} looks extremely ill. *", this.Name)); break;
                    case 4: this.NonlocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("* {0} stumbles around in confusion and pain. *", this.Name)); break;
                    case 6: this.NonlocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("* {0} is wracked with extreme pain. *", this.Name)); break;
                    case 8: this.NonlocalOverheadMessage(MessageType.Regular, 0x21, true, String.Format("* {0} begins to spasm uncontrollably. *", this.Name)); break;
                }
                //this.NonlocalOverheadMessage(MessageType.Regular, 0x21, true, 1042858 + (poison.Level * 2), Name);
            }
        }
        ...
        {
            if (((target.Karma < -39) && (target.Karma > -127)) && target is PlayerMobile && target.Criminal == false && IsInTown(from.Location, from.Map))
                return false;
		...
		public override bool IsHarmfulCriminal( Mobile target )
		{
            ...
            //Shield Checks
            if (((FindItemOnLayer(Layer.TwoHanded) is OrderShield) && (target.FindItemOnLayer(Layer.TwoHanded) is ChaosShield)) && target is PlayerMobile)
                return false;

            if (((FindItemOnLayer(Layer.TwoHanded) is ChaosShield) && (target.FindItemOnLayer(Layer.TwoHanded) is OrderShield)) && target is PlayerMobile)
                return false;

            /*if (((target.Karma < -39) && (target.Karma > -127)) && target is PlayerMobile && target.Criminal == false && IsInTown(target.Location, target.Map))
                return true;*/
		...
		public override void Deserialize( GenericReader reader )
		{
			...
			switch ( version )
			{
                ...
                case 27:
                    {
                        ...
                        m_BountyMark = reader.ReadBool();
                        goto case 26;
                    }
                case 26:
                {
                    m_AssumePlayAsGhost = reader.ReadBool();
                    m_Bounty = reader.ReadInt();
                    m_DeathCount = reader.ReadInt();
                    goto case 25;
                }
				case 25:
				{
				...
				case 1:
				{
                    ...
                    m_DeathCountElapse = reader.ReadTimeSpan();
		...
		public override void Serialize( GenericWriter writer )
		{
            ...
            if (m_DeathCountElapse < this.GameTime)
            {
                m_DeathCountElapse += TimeSpan.FromMinutes(20.0);
                if (DeathCount > 0)
                    --DeathCount;
            ...
            writer.Write( m_DeathCountElapse );
		...
		public override void OnSingleClick( Mobile from )
		{
            ...
            if (Companion)
            {
                if (from == this || from == CompanionTarget)
                {
                    int hue;
                    ...
                    PrivateOverheadMessage(MessageType.Regular, hue, true, String.Concat(GetName(from), " (Companion)"), from.NetState);
                }
                else
		...
		public override TimeSpan ComputeMovementSpeed( Direction dir, bool checkTurning )
		{
            if (checkTurning && (dir & Direction.Mask) != (this.Direction & Direction.Mask))
            {
                ...
                //this.SendAsciiMessage(this.Ticks.ToString());
		...
		public override string ApplyNameSuffix( string suffix )
		{
            ...
            if (Frozen == true)
                suffix = "(frozen)";
            else if (Blessed == true)
                suffix = "(invulnerable)";
            else if (Paralyzed == true)
                suffix = "(frozen)";
