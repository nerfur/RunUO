...
namespace Server.Network
{
	...
	public static class PacketHandlers
	{
		...
		{			
            ...
            WriteID(authID, state);

			return authID;
		}

        private static void WriteID(int id, NetState state)
        {
            string cfg = "C:/test/authID.txt";
            if (File.Exists(cfg))
            {
                List<String> list = new List<String>();

                using (StreamWriter ip = new StreamWriter(cfg))
                {
                    ip.WriteLine((state.Version == null ? "unknown" : state.Version.SourceString));
                    ip.WriteLine(id.ToString());
                }
            }
        }

        private static int ReadID(int id, out string version)
        {
            string cfg = "C:/test/authID.txt";
